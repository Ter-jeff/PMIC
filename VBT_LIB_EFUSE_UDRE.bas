Attribute VB_Name = "VBT_LIB_EFUSE_UDRE"
Option Explicit

''''20160805 update for UDRE_Ver1 UDRE CMPE Fuse
Public Function auto_CMPE_Syntax_Chk(CMPE_pat As Pattern, OutPin As PinList, Optional Validating_ As Boolean) As Long

On Error GoTo errHandler
    Dim funcName As String:: funcName = "auto_CMPE_Syntax_Chk"
    ...
    If (auto_eFuse_PatSetToPat_Validation(CMPE_pat, ReadPatt, Validating_) = True) Then Exit Function
    ''''----------------------------------------------------------------------------------------------------

    Dim site As Variant
    Dim Trim_code_CMPE As New DSPWave
    Dim CMPE_BitStr As New SiteVariant
    Dim i As Long, j As Long, k As Long
    Dim PatCMPEArray() As String
    ...
    Dim CMPE_PrintRow As Long
    Dim CMPE_BitPerRow As Long
    Dim CMPE_CapBits As Long
    Dim CMPE_TotalBit As Long
    Dim allBlank As New SiteBoolean
    Dim blank_stage As New SiteBoolean
    ''Dim SiteVarValue As Long
    
    Dim m_catenameUDRE As String ''''UDRE_Fuse Category name
    ...
    Dim m_defvalUDRE As Variant ''''20160905
    Dim m_HexStr As String

    Dim Flag_CMPECategoryMatch As Boolean
    
    CMPE_CapBits = gL_CMPE_DigCapBits_Num
    CMPE_TotalBit = gL_CMPE_DigCapBits_Num
    ...
    ''''20150731 update
    CMPE_PrintRow = IIf((CMPE_TotalBit Mod CMPE_BitPerRow) > 0, Floor(CMPE_TotalBit / CMPE_BitPerRow) + 1, Floor(CMPE_TotalBit / CMPE_BitPerRow))
    ReDim DigCapArray((CMPE_PrintRow * CMPE_BitPerRow) - 1)
...
If (gB_eFuse_newMethod) Then
    TheHdw.Patterns(CMPE_pat).Load
    TheHdw.Digital.ApplyLevelsTiming True, True, True, tlPowered

    Status = GetPatListFromPatternSet(CMPE_pat.Value, PatCMPEArray, pat_count)
    TheExec.Datalog.WriteComment ""

    Call auto_eFuse_DSSC_DigCapSetup(PatCMPEArray(0), OutPin, "CMPE_cap", CMPE_CapBits, Trim_code_CMPE)
    Call TheHdw.Patterns(PatCMPEArray(0)).Test(pfAlways, 0)
    gStr_PatName = PatCMPEArray(0)
    Call UpdateDLogColumns(gI_CMPE_catename_maxLen)
    ...
    If (TheExec.TesterMode = testModeOffline) Then
        ...
        Call auto_CMP_Sim_New(eFuse_CMPE, True)
...
'        Call auto_eFuse_print_capWave32Bits(eFuse_CMP, Trim_code_CMP, False) ''''True to print out
        For Each site In TheExec.sites
            Trim_code_CMPE = gDW_CMPE_Pgm_SingleBitWave.Copy
        Next site
    End If
    ...
    Call auto_eFuse_DSSC_ReadDigCap_32bits_NEW(m_Fusetype, 1, Trim_code_CMPE, m_FBC, blank_stage, allBlank, True)
    ...
    'Call auto_eFuse_CMPE_Parsing_HLlimit(m_Fusetype)
...
'        Call auto_eFuse_DSSC_DigCapSetup(PatCMPEArray(j), OutPin, "CMPE_cap", CMPE_CapBits, Trim_code_CMPE)
'        Call TheHdw.Patterns(PatCMPEArray(j)).test(pfAlways, 0)
'
'        Call UpdateDLogColumns(gI_CMPE_catename_maxLen)
'
'        For Each Site In TheExec.Sites
'
'            CMPE_BitStr(Site) = ""
'            gB_CMPE_decode_flag(Site) = False
'
...
'
'            ''''20150717 update
'            ''''composite to the CMPE_BitStr() from the DSSC Capture
'            If (UCase(gS_CMPE_PatBitOrder) = "MSB") Then
'                ''''<Notice>
'                ''''the pattern DSSC capture from MSB to LSB (bitLast to bit0)
'                ''''so Trim_code_CMPE.Element(0) is MSB
'                ''''DigCapArray(0) Must be 'bit0' for the function auto_PrintAllBitbyDSSC()
'                For i = 0 To CMPE_CapBits - 1
'                    DigCapArray(i) = Trim_code_CMPE.Element(CMPE_CapBits - 1 - i)    ''''Reverse Bit String
'                    CMPE_BitStr(Site) = CMPE_BitStr(Site) & Trim_code_CMPE.Element(i) ''''[MSB ... LSB]
'                Next i
'            Else
'                ''''<Notice>
'                ''''the pattern DSSC capture from LSB to MSB (bit0 to bitLast)
'                ''''so Trim_code_CMPE.Element(0) is LSB
'                ''''DigCapArray(0) Must be 'bit0' for the function auto_PrintAllBitbyDSSC()
'                For i = 0 To CMPE_CapBits - 1
'                    DigCapArray(i) = Trim_code_CMPE.Element(i)
'                    CMPE_BitStr(Site) = DigCapArray(i) & CMPE_BitStr(Site) ''''[MSB ... LSB]
'                Next i
'            End If
'
'            ''''20160324 update
...
'                CMPE_BitStr(Site) = auto_CMP_Sim_New(True) ''''<Notice> CMPE_BitStr MUST be [MSB ... LSB] [bitLast...bit0]
'                For i = 0 To CMPE_CapBits - 1
'                    DigCapArray(i) = CLng(Mid(StrReverse(CMPE_BitStr(Site)), i + 1, 1))
'                Next i
'            End If
...
'            '=======================================================
'            TheExec.Datalog.WriteComment ""
'            Call auto_PrintAllBitbyDSSC(DigCapArray, CMPE_PrintRow, UBound(DigCapArray) + 1, CMPE_BitPerRow)
'
'            TheExec.Datalog.WriteComment "Site(" & Site & "), CMPE pat:" & PatCMPEArray(j) & ", Shift out code [" + CStr(CMPE_CapBits - 1) + ":0]=" + CMPE_BitStr(Site)
'            TheExec.Datalog.WriteComment ""
'
'            Dim Count_1s As Long
'            Count_1s = 0
'            For i = 1 To Len(CMPE_BitStr(Site))
'                If (CStr(Mid(CMPE_BitStr(Site), i, 1)) = "1") Then
'                    Count_1s = Count_1s + 1
'                End If
'            Next i
'
'            TheExec.Datalog.WriteComment "Site(" & Site & "), CMPE pat:" & PatCMPEArray(j) & " UDREVer1 total 1s amount = " & CStr(Count_1s)
'
...
'            Call auto_Decode_CMPE_Binary_Data(DigCapArray)
'
'            ''''judge limit
'            For i = 0 To UBound(CMPE_Fuse.Category)
'                tmpdlgStr = ""
'                m_catename = CMPE_Fuse.Category(i).Name
'                m_algorithm = LCase(CMPE_Fuse.Category(i).Algorithm)
'                m_startbit = CMPE_Fuse.Category(i).SeqStart
'                m_endbit = CMPE_Fuse.Category(i).SeqEnd
'                m_bitwidth = CMPE_Fuse.Category(i).Bitwidth
'                m_decimal = CMPE_Fuse.Category(i).Read.Decimal(Site)
'                m_value = CMPE_Fuse.Category(i).Read.Value(Site)
'                m_hexStr = CMPE_Fuse.Category(i).Read.HexStr(Site)
'
'                ''''initialize everytime
'                Flag_CMPECategoryMatch = False
'                m_lolmtV = -1
'                m_hilmtV = m_lolmtV
'
'                For k = 0 To UBound(UDRE_Fuse.Category)
'                    m_catenameUDRE = UCase(UDRE_Fuse.Category(k).Name)
'                    If (m_catenameUDRE = UCase(m_catename)) Then
'                        ''m_defvalUDRE = UDRE_Fuse.Category(k).DefaultValue ''''20160905
'                        m_lolmtV = UDRE_Fuse.Category(k).Read.Decimal(Site)
'                        m_hilmtV = m_lolmtV
'
...
'''''                        End If
'
'                        Flag_CMPECategoryMatch = True
'                        Exit For
'                    End If
'                Next k
'
'                If Flag_CMPECategoryMatch = False Then
'                    TheExec.Datalog.WriteComment ("====================================================================")
'                    TheExec.Datalog.WriteComment ("    Can't CMPE Category : " & m_catename & " in UDRE Category!!!")
'                    TheExec.Datalog.WriteComment ("====================================================================")
'                End If
'
'                ''''20170811 update
...
Public Function auto_UDRE_USI(USI_pat As Pattern, InPin As PinList, Optional condstr As String = "stage", Optional Validating_ As Boolean) As Long

On Error GoTo errHandler
    Dim funcName As String:: funcName = "auto_UDRE_USI"
...
Public Function auto_UDRE_USO_Syntax_Chk(Optional condstr As String = "all") As Long

On Error GoTo errHandler
    Dim funcName As String:: funcName = "auto_UDRE_USO_Syntax_Chk"
...
'
'    USO_CapBits = gL_UDRE_USO_DigCapBits_Num
'    USO_TotalBit = gL_UDRE_USO_DigCapBits_Num
'
'    ''''display bit number per row
'    If (gS_EFuse_Orientation = "UP2DOWN") Or (gS_EFuse_Orientation = "SingleUp") Then
'        USO_BitPerRow = 32
...
'            gL_eFuse_Sim_Blank = 1
'        End If
'        Call RunDSP.eFuse_SingleBitWave2CapWave32Bits(eFuse_UDRE, Trim_code_USO)
'        Call auto_eFuse_print_capWave32Bits(eFuse_UDRE, Trim_code_USO, False) ''''True to print out
'    End If
'
'    If (condstr = "cp1_early") Then
'        m_bitFlag_mode = 0
...
'
'    If (gL_UDRE_USO_PatBitOrder = "LSB") Then
...
'    End If
    ...
    Call auto_eFuse_SyntaxCheck(m_Fusetype, condstr)
            ...
            ''''----------------------------------------------------------------------------------------------
    If (gS_JobName <> "cp1_early") Then
        Dim m_UDRE_DATA_STR As New SiteVariant
        For Each site In TheExec.sites
            DoubleBitArray = gDW_UDRE_Read_DoubleBitWave.Data
            
            m_UDRE_DATA_STR(site) = "" ''''is a String [(bitLast)......(bit0)]
            
            For i = 0 To UBound(DoubleBitArray)
                m_UDRE_DATA_STR(site) = CStr(DoubleBitArray(i)) + m_UDRE_DATA_STR(site)
            Next i
            ...
            Call auto_eFuse_to_STDF_allBits("UDRE", m_UDRE_DATA_STR(site))
        Next site
    End If
    ''''----------------------------------------------------------------------------------------------
...
''''20150731 New
Public Function auto_UDRE_USO_BlankChk_byStage(USO_pat As Pattern, OutPin As PinList, _
                    Optional m_decode_flag As Boolean = True, _
                    Optional condstr As String = "stage", _
                    Optional Validating_ As Boolean) As Long

On Error GoTo errHandler
    Dim funcName As String:: funcName = "auto_UDRE_USO_BlankChk_byStage"
    ...
    ReDim gL_Sim_FuseBits(TheExec.sites.Existing.Count - 1, gL_UDRE_USO_DigCapBits_Num - 1) ''''it's for the simulation
    
    USO_CapBits = gL_UDRE_USO_DigCapBits_Num
    USO_TotalBit = gL_UDRE_USO_DigCapBits_Num

    ''''it's used to identify if the Job Name is existed in the UDRE portion of the eFuse BitDef table.
    m_jobinStage_flag = auto_eFuse_JobExistInStage("UDRE", True) ''''<MUST>
...
If (gB_eFuse_newMethod = True) Then
    ...
    Call UpdateDLogColumns(gI_UDRE_catename_maxLen) ''''<MUST>must after execute pattern
...
'            gL_eFuse_Sim_Blank = 1
'        End If
        gDW_UDRE_Pgm_SingleBitWave = Temp_USO.Copy
        Call rundsp.eFuse_SingleBitWave2CapWave32Bits(eFuse_UDRE, Temp_USO)
...
'        m_cmpResult = -1
    ...
    For Each site In TheExec.sites
        If (auto_eFuse_GetAllPatTestPass_Flag("UDRE") = True And m_FBC = 0) Then
...
'                End If
'                Call eFuseENGFakeValue_Sim
'                Call auto_UDRE_USI_Sim(blank_stage(Site), False) ''''True for print debug
'                USO_BitStr(Site) = gS_UDRE_USI_BitStr(Site)
'                For i = 0 To USO_CapBits - 1
'                    DigCapArray(i) = CLng(Mid(StrReverse(gS_UDRE_USI_BitStr(Site)), i + 1, 1))
...
'            testName = "UDRE_BlankChk_" + UCase(gS_JobName)
'            SingleDoubleFBC = 0 ''''init
'            Call auto_eFuse_BlankChk_FBC_byStage("UDRE", DigCapArray, blank_stage, SingleDoubleFBC)
'
'            '=======================================================
'            '= Print out the caputured bit data from DigCap        =
'            '=======================================================
...
'                    If (auto_eFuse_GetAllPatTestPass_Flag("UDRE") = False) Then
'                        ResultFlag(Site) = 1   ''Fail then NO fuse
'                        PinName = "Fail"
'                        SiteVarValue = 0
'                    Else
...
'            If (SiteVarValue <> 1 And m_decode_flag) Then Call auto_Decode_UDRE_Binary_Data(DigCapArray)
'
'            gB_UDRE_decode_flag(Site) = False ''''20160531, <MUST> reset and init
'
'            ''''------------------------------------------------------------------------
'            ''''<Very Important>
...
'            If (TheExec.Sites.Item(Site).FlagState("F_udre_blank") = logicTrue) Then
'                ''''Here it means that UFR read mode pattern is failure
'                TheExec.Datalog.WriteComment "<WARNING> " + funcName + ":: Site(" + CStr(Site) + ") UDRE_UFR_NV Function Failure"
...
Public Function auto_UDRE_USO_Read_Decode(USO_pat As Pattern, OutPin As PinList, Optional Validating_ As Boolean) As Long

On Error GoTo errHandler
    Dim funcName As String:: funcName = "auto_UDRE_USO_Read_Decode"
...
'    End If
    ...
    ''''20170111 Add
    Call auto_eFuse_ReadAllData_to_DictDSPWave("UDRE", False, False)
...
'            Call auto_PrintAllBitbyDSSC(DigCapArray, USO_PrintRow, UBound(DigCapArray) + 1, USO_BitPerRow)
'
'            Call auto_Decode_UDRE_Binary_Data(DigCapArray, Not allblank(Site)) ''''True for Debug, only Not allBlank to show the decode result
'
'            ''Binning out
...
Public Function auto_UDRE_USO_BlankChk_Early_byStage(USO_pat As Pattern, OutPin As PinList, Optional m_decode_flag As Boolean = True, Optional Validating_ As Boolean) As Long

On Error GoTo errHandler
    Dim funcName As String:: funcName = "auto_UDRE_USO_BlankChk_Early_byStage"
    ...
    ''''20180105 New
    ''''It's used to check if gS_JobName is existed in all UDRE_Fuse programming stages
    ''''it's used to identify if the Job Name is existed in the UDRE portion of the eFuse BitDef table.
    Dim m_jobinStage_flag As Boolean
    m_jobinStage_flag = auto_eFuse_JobExistInStage("UDRE", True) ''''<MUST>
...
'            Next i
'
'            testName = "UDRE_BlankChk_CP1_Early" ''''"UDRE_BlankChk_" + UCase(gS_JobName)
'
'            Call auto_UDRE_blank_check_Early(DigCapArray, blank_early, blank_stage_noEarly)
...
Public Function auto_UDRE_USI_Sim(m_blank As Boolean, Optional showPrint As Boolean = False) As Long

On Error GoTo errHandler
    Dim funcName As String:: funcName = "auto_UDRE_USI_Sim"
    ...
    For Each site In TheExec.sites
        ...
        For i = 0 To UBound(UDRE_Fuse.Category)
            ...
            ElseIf (m_algorithm = "vddbin") Then
                ...
                If (m_defreal = "decimal") Then ''''20160624 update
                    m_decimal = auto_eFuse_Get_DefaultRealDecimal("UDRE", m_catename, m_defreal, m_defval, False)
                Else
                    m_decimal = auto_Vbin_to_VfuseStr_New(tmpVbin, m_bitwidth, tmpVfuse, m_resolution)
                End If
            ...
            ''''-------------------------------------------------------------------------------------------------------
            ''''20150825 update
            Call auto_eFuse_Dec2PgmArr_Write_byStage("UDRE", m_stage, i, m_decimal, m_LSBbit, m_MSBBit, PgmBitArr, False) ''''False for the Simulation
    ...
    If (False) Then
        For Each site In TheExec.sites
            TheExec.Datalog.WriteComment funcName + ", Site(" + CStr(site) + "), Shift In Code [" + CStr(UBound(PgmBitArr)) + ":0]=" + gS_UDRE_USI_BitStr(site)
        Next site
    End If
...
Public Function auto_UDRE_USO_COMPARE(USO_pat As Pattern, OutPin As PinList, Optional condstr As String = "all", Optional Validating_ As Boolean) As Long

On Error GoTo errHandler
    Dim funcName As String:: funcName = "auto_UDRE_USO_COMPARE"
...
'    Next
    ...
    If (TheExec.TesterMode = testModeOffline) Then
        ...
        End If
        Call rundsp.eFuse_SingleBitWave2CapWave32Bits(eFuse_UDRE, Temp_USO)
        Call auto_eFuse_print_capWave32Bits(eFuse_UDRE, Temp_USO, False) ''''True to print out
        If (m_PatBitOrder = "bit0_bitLast") Then
            For Each site In TheExec.sites
                Trim_code_USO(site) = Temp_USO(site).Copy
            Next
...
'''''''''20150731 New
'''''Public Function auto_UDRE_USO_BlankChk(USO_pat As Pattern, OutPin As PinList, _
'''''                                       BankStr As String, _
'''''                                       Optional condstr As String = "stage", _
'''''                                       Optional Validating_ As Boolean) As Long
'''''
'''''On Error GoTo errHandler
'''''    Dim funcName As String:: funcName = "auto_UDRE_USO_BlankChk"
'''''
'''''    ''''----------------------------------------------------------------------------------------------------
'''''    ''''<Important>
'''''    ''''Must be put before all implicit array variables, otherwise the validation will be error.
...
'''''    Dim SingleDoubleFBC As Long
'''''    Dim m_jobinStage_flag As Boolean
'''''    Dim m_BankStr As String
'''''    Dim m_FBC As New SiteLong ''''Fail bit count of the singleBit and doobleBit
'''''    Dim m_Fusetype As eFuseBlockType
'''''    Dim m_SiteVarValue As New SiteLong
'''''    Dim m_ResultFlag As New SiteLong
'''''    Dim m_PatBitOrder As String
'''''    Dim m_USO_Order As String
'''''    Dim m_siteVar As String
'''''
'''''
'''''    If (BankStr = "UDR") Then
...
'''''        m_USO_Order = gL_USO_PatBitOrder
'''''    ElseIf (BankStr = "UDRE") Then
'''''        m_siteVar = "UDR_EChk_Var"
'''''        m_Fusetype = eFuse_UDRE
'''''        USO_CapBits = gL_UDRE_USO_DigCapBits_Num
'''''        m_USO_Order = gL_UDRE_USO_PatBitOrder
'''''    ElseIf (BankStr = "UDRP") Then
'''''        m_siteVar = "UDR_PChk_Var"
'''''        m_Fusetype = eFuse_UDRP
'''''        USO_CapBits = gL_UDRP_USO_DigCapBits_Num
'''''        m_USO_Order = gL_UDRP_USO_PatBitOrder
'''''    End If
'''''
'''''
'''''
...
'''''
'''''    ''''it's used to identify if the Job Name is existed in the UDRE portion of the eFuse BitDef table.
'''''    m_jobinStage_flag = auto_eFuse_JobExistInStage(BankStr, True) ''''<MUST>
'''''
'''''    ''''display bit number per row
''''''    If (gS_EFuse_Orientation = "UP2DOWN") Or (gS_EFuse_Orientation = "SingleUp") Then
''''''        USO_BitPerRow = 32
...
''''''            gL_eFuse_Sim_Blank = 1
''''''        End If
'''''        Call RunDSP.eFuse_SingleBitWave2CapWave32Bits(m_Fusetype, Temp_USO)
'''''        'Call auto_eFuse_print_capWave32Bits(eFuse_UDR, Temp_USO, False) ''''True to print out
'''''        If (m_PatBitOrder = "bit0_bitLast") Then
'''''            For Each Site In TheExec.Sites
'''''                Trim_code_USO(Site) = Temp_USO(Site).Copy
...
'''''    ''''Here Parameter bitFlag_mode=1 (Stage Bits)
'''''    'gL_eFuse_Sim_Blank = 1
'''''    Call auto_eFuse_DSSC_ReadDigCap_32bits_NEW(m_Fusetype, m_bitFlag_mode, Trim_code_USO, m_FBC, blank_stage, allBlank, m_SerialType, m_PatBitOrder)
'''''
'''''    If (blank_stage.Any(False) = True) Then
'''''        ''''201901XX New for TTR/PTE improvement
'''''        Call auto_eFuse_setReadData_forSyntax(m_Fusetype)
...
'''''
'''''    For Each Site In TheExec.Sites
'''''        If (auto_eFuse_GetAllPatTestPass_Flag(BankStr) = True And m_FBC = 0) Then
'''''            m_ResultFlag = 0  'Pass Blank check criterion
'''''        Else
'''''            m_ResultFlag = 1  'Fail Blank check criterion
'''''            m_SiteVarValue = 0
...
'''''    TheExec.Flow.TestLimit resultVal:=m_SiteVarValue, lowval:=1, hival:=2, Tname:=m_siteVar '', PinName:="Value"
'''''
'''''    testName = BankStr + "_FailBitCount_" + UCase(gS_JobName)
'''''    TheExec.Flow.TestLimit resultVal:=m_FBC, lowval:=0, hival:=0, Tname:=testName '', PinName:="Value"
'''''
'''''    testName = BankStr + "_Blank_" + UCase(gS_JobName)
'''''    TheExec.Flow.TestLimit resultVal:=m_ResultFlag, lowval:=0, hival:=0, Tname:=testName ', PinName:=PinName
'''''
'''''    Call UpdateDLogColumns__False
'''''    'DebugPrintFunc ReadPatSet.Value
