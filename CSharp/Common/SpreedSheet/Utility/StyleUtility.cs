...
namespace unvell.ReoGrid.Utility
{
    ...
    public sealed class StyleUtility
    {
        ...
        internal static WorksheetRangeStyle DistinctStyle(WorksheetRangeStyle style, WorksheetRangeStyle referStyle)
        {
            ...
            if (distinctedFlag == PlainStyleFlag.None) return null;

            var distinctedStyle = new WorksheetRangeStyle();
            CopyStyle(style, distinctedStyle, distinctedFlag);
        ...
        internal static WorksheetRangeStyle CheckAndRemoveCellStyle(Worksheet grid, Cell cell)
        {
            ...
            var row = cell.InternalRow;
            var col = cell.InternalCol;

            var style = cell.InnerStyle;

            var pKind = StyleParentKind.Own;
            var defaultStyle = FindCellParentStyle(grid, row, col, out pKind);
        ...
        internal static WorksheetRangeStyle FindCellParentStyle(Worksheet sheet, int row, int col,
            out StyleParentKind pKind)
        {
            var rowhead = sheet.RetrieveRowHeader(row);
            ...
            var colhead = sheet.RetrieveColumnHeader(col);
        ...
        {
            var rowhead = grid.RetrieveRowHeader(cell.InternalRow);
            ...
            {
                var colhead = grid.RetrieveColumnHeader(cell.InternalCol);
        ...
        internal static RGXmlCellStyle ConvertToXmlStyle(WorksheetRangeStyle style)
        {
            ...
            var xmlStyle = new RGXmlCellStyle();
            ...
            {
                var xmlFillPattern = new RGXmlCellStyleFillPattern
                {
            ...
            CultureInfo culture)
        {
            ...
            if (!string.IsNullOrEmpty(xmlStyle.indent))
            {
                ...
                var indent = TextFormatHelper.GetPixelValue(xmlStyle.indent, 0);
                if (indent > 0 && indent < 65535) style.Indent = (ushort)indent;
            }
        ...
        {
            if (cell.InnerStyle != null && cell.InnerStyle.HAlign != GridHorAlign.General)
        ...
        {
            var fontStyle = FontStyles.Regular;
