...
#region Rows & Cols
			this["rows"] =
 new ExternalProperty(() => { return sheet.RowCount; }, (v) => { sheet.SetRows(ScriptRunningMachine.GetIntValue(v)); });
			this["cols"] =
 new ExternalProperty(() => { return sheet.ColumnCount; }, (v) => { sheet.SetCols(ScriptRunningMachine.GetIntValue(v)); });
...
#region Selection
	internal class RSSelectionObject : RSWorksheet
	{
			...
			: base(sheet)
		{
			...
			this["toString"] =
 new NativeFunctionObject("toString", (ctx, owner, args) => sheet.SelectionRange.ToString());
			this["toAddress"] =
 new NativeFunctionObject("toAddress", (ctx, owner, args) => sheet.SelectionRange.ToAddress());
			this["toSpans"] =
 new NativeFunctionObject("toSpans", (ctx, owner, args) => sheet.SelectionRange.ToStringSpans());
	...
	internal class RSCellStyleObject : ObjectValue
	{
		...
		public RSCellStyleObject(Worksheet sheet, Cell cell)
		{
			...
			this["fontSize"] = new ExternalProperty(
				() => cell.Style.FontSize,
				(v) => cell.Style.FontSize =
 TextFormatHelper.GetFloatPixelValue(ScriptRunningMachine.ConvertToString(v),
