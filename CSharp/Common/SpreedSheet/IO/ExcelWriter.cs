...
namespace unvell.ReoGrid.IO.OpenXML
{
    ...
    internal sealed class ExcelWriter
    {
        public static void WriteStream(RGWorkbook rgWorkbook, Stream stream)
        {
            ...
            foreach (var rgSheet in rgWorkbook.Worksheets)
            {
                ...
                {
                    if (workbook.definedNames == null) workbook.definedNames = new List<DefinedName>();
        ...
        private static int WriteFill(Document doc, SolidColor c)
        {
            ...
            if (existedIndex >= 0) return existedIndex;
        ...
        private static int WriteFont(Document doc, WorksheetRangeStyle rgStyle)
        {
            ...
            var existedIndex = styles.fonts.FindIndex(f =>
                ...
                && (((flag & PlainStyleFlag.FontStyleItalic) == PlainStyleFlag.FontStyleItalic && f.italic != null &&
                     f.italic != null)
                    || ((flag & PlainStyleFlag.FontStyleItalic) != PlainStyleFlag.FontStyleItalic && f.italic == null))
                && (((flag & PlainStyleFlag.FontStyleStrikethrough) == PlainStyleFlag.FontStyleStrikethrough &&
                     f.strikethrough != null && f.strikethrough != null)
                    || ((flag & PlainStyleFlag.FontStyleStrikethrough) != PlainStyleFlag.FontStyleStrikethrough &&
                        f.strikethrough == null))
                && (((flag & PlainStyleFlag.FontStyleUnderline) == PlainStyleFlag.FontStyleUnderline &&
                     f.underline != null && f.underline != null)
                    || ((flag & PlainStyleFlag.FontStyleUnderline) != PlainStyleFlag.FontStyleUnderline &&
                        f.underline == null))
            ...
            styles.fonts.Add(new Font(rgStyle));
            ...
            RangeBorderStyle? left, RangeBorderStyle? right)
        {
            ...
            var existedIndex = styles.borders.FindIndex(_b
                => ((_b.top != null && top != null && _b._top == top.Value) || (_b.top == null && top == null))
                   && ((_b.bottom != null && bottom != null && _b._bottom == bottom.Value) ||
                       (_b.bottom == null && bottom == null))
                   && ((_b.left != null && left != null && _b._left == left.Value) || (_b.left == null && left == null))
                   && ((_b.right != null && right != null && _b._right == right.Value) ||
                       (_b.right == null && right == null)));
            ...
            string prefix = null, string postfix = null)
        {
            ...
            if (arg.UseSeparator) sbDigits.Append("#,##");
            ...
            if (arg.DecimalPlaces > 0)
            {
                ...
                for (var i = 0; i < arg.DecimalPlaces; i++) sbDigits.Append('0');
            }

            var digits = sbDigits.ToString();
            ...
            switch (arg.NegativeStyle)
            {
                ...
                case NumberDataFormatter.NumberNegativeStyle.RedMinus:
                    if (!string.IsNullOrEmpty(prefix)) sb.Append(prefix);
                    sb.Append("[Red]");
                    if ((arg.NegativeStyle & NumberDataFormatter.NumberNegativeStyle.Minus) ==
                        NumberDataFormatter.NumberNegativeStyle.Minus) sb.Append("-");
                    sb.Append(digits);
                    if (!string.IsNullOrEmpty(postfix)) sb.Append(postfix);
                    break;
        ...
        {
            if (styles.numberFormats == null) styles.numberFormats = new NumberFormatCollection();
            ...
            switch (flag)
            {
...

                {
                    ...
                    for (var i = 0; i < styles.numberFormats.list.Count; i++)
                    {
                    ...
                    var formatCode = ConvertToExcelNumberPattern(narg, prefix, postfix);
                ...
                case CellDataFormatFlag.DateTime:
                    ...
                    switch (dtarg.Format)
                    {
                        ...
                        default:
                        {
                            var id = styles.numberFormats.Count + BaseUserNumberFormatId;
        ...
        private static string ConvertToExcelHorAlign(GridHorAlign halign)
        {
            switch (halign)
            {
                default:
        ...
        private static string ConvertToExcelVerAlign(GridVerAlign valign)
        {
            switch (valign)
            {
                default:
            ...
            CellDataFormatFlag dataFormatFlag = CellDataFormatFlag.General, object dataFormatArg = null)
        {
            ...
            {
                var numFmt = WriteNumberFormat(dataFormatFlag, dataFormatArg, doc.Stylesheet);

                if (numFmt > 0) numberFormatId = numFmt.ToString();
            }
            ...
            if (rgStyle != null)
            {
                if ((rgStyle.Flag & PlainStyleFlag.HorizontalAlign) == PlainStyleFlag.HorizontalAlign
                    && rgStyle.HAlign != GridHorAlign.General)
                {
                    horAlign = true;

                    if ((rgStyle.Flag & PlainStyleFlag.Indent) == PlainStyleFlag.Indent) indent = rgStyle.Indent;
                }
                ...
                if ((rgStyle.Flag & PlainStyleFlag.VerticalAlign) == PlainStyleFlag.VerticalAlign
                    ...
                    && rgStyle.VAlign != GridVerAlign.General
                    && rgStyle.VAlign != GridVerAlign.Bottom)
                    verAlign = true;
            ...
            var existedStyleIndex = styles.cellFormats.FindIndex(s =>
                s.fillId == fillId
                && s.fontId == fontId
            ...
            if (existedStyleIndex >= 0) return existedStyleIndex;
            ...
            if (horAlign || verAlign || textWrap || textRotate)
            {
                ...
                if (horAlign)
                {
                    ...
                    if (rgStyle.HAlign == GridHorAlign.Left) align.indent = Convert.ToString(rgStyle.Indent);
                }
                ...
                if (textWrap) align.wrapText = "1";

                if (textRotate)
                {
                    align._rotateAngle = (int)rgStyle.RotationAngle;
                    align.textRotation =
                        (rgStyle.RotationAngle < 0 ? Math.Abs(rgStyle.RotationAngle - 90) : rgStyle.RotationAngle)
                        .ToString();
                }
            ...
            Debug.Assert(fillId != null || fontId != null || numberFormatId != null || borderId != null || !horAlign ||
                         !verAlign);
        ...
        private static void WriteDefaultStyles(Document doc, RGWorkbook rgWorkbook)
        {
            ...
            style.fonts.Add(new Font(defaulStyle));
        ...
        {
            if (doc.SharedStrings == null) doc.SharedStrings = doc.CreateSharedStrings();
        ...
        private static void WriteWorksheet(Document doc, RGWorksheet rgSheet)
        {
            ...
            var maxRows = Math.Max(rgSheet.MaxContentRow, 0);
            var maxCols = Math.Max(rgSheet.MaxContentCol, 0);
            ...
            {
                address = dimensionRange.IsSingleCell
                    ? dimensionRange.StartPos.ToRelativeAddress()
                    : dimensionRange.ToRelativeAddress()
            };

            sheet.sheetViews = new List<SheetView>(new[]
            {
                new SheetView
                {
                    tabSelected = rgSheet.controlAdapter == null
                        ? rgSheet.workbook.GetWorksheetIndex(rgSheet) == 0 ? "1" : "0"
                        : rgSheet == rgSheet.controlAdapter.ControlInstance.ActiveWorksheet
                            ? "1"
                            : "0",
                    workbookViewId = "0",
            ...
            var lastColAutoWidth = firstColHeader.IsAutoWidth;
            ...
            for (var i = 1; i <= maxCols; i++)
            {
                ...
                {
                    var customWidth = lastColWidth != rgSheet.DefaultColumnWidth;

                    if (sheet.cols == null) sheet.cols = new List<Column>();
            ...
            if (lastColIndex > 0 && lastColIndex < rgSheet.Columns)
            {
                ...
                sheet.cols.Add(new Column
                {
                    ...
                    max = rgSheet.Columns,
                    width =
                        Math.Truncate((rgSheet.DefaultColumnWidth - columnWidthPad) / fixedCharWidth * 100.0 + 0.5) /
                        100.0
                });
            }
            ...
            for (var r = 0; r <= maxRows; r++)
            {
                ...
                for (var c = 0; c <= maxCols;)
                {
                    ...
                    var dflag = CellDataFormatFlag.General;
                    object dfarg = null;
                    ...
                    if (rgCell != null)
                    {
                        ...
                        else
                        {
                            ...
                            var hasFormula = rgCell.HasFormula;
                            ...
                            else if (data is DateTime)
                            {
                                ...
                                var days = (dt - DateTimeDataFormatter.BaseStartDate).TotalDays + 1;
                                if (days > 59) days++;
                            ...
                            else if (data != null)
                            {
                                ...
                                if (str.Length > 0)
                                {
                                    ...
                                    {
                                        var sharedStrId = AddSharedString(doc, str);
                                        cell.value = new ElementText(sharedStrId.ToString());
                                        cell.dataType = "s";
                                    }
                                }
                            ...
                            // formula
                            if (hasFormula) cell.formula = new Formula { val = rgCell.InnerFormula };
                            ...
                            {
                                if (sheet.mergeCells == null) sheet.mergeCells = new List<MergeCell>();

                                sheet.mergeCells.Add(new MergeCell
                                {
                                    address = new RangePosition(rgCell.MergeStartPos, rgCell.MergeEndPos).ToAddress()
                                });
                    ...
                    var top = rgSheet.GetValidHBorderByPos(r, c, HBorderOwnerPosition.Top);
                    var bottom = rgSheet.GetValidHBorderByPos(r + 1, c, HBorderOwnerPosition.Bottom);
                    var left = rgSheet.GetValidVBorderByPos(r, c, VBorderOwnerPosition.Left);
                    var right = rgSheet.GetValidVBorderByPos(r, c + 1, VBorderOwnerPosition.Right);
                    ...
                    if (row.cells == null) row.cells = new List<Schema.Cell>();
    ...
    internal partial class Document : OpenXMLFile
    {
        ...
        internal Schema.Workbook CreateWorkbook()
        {
            ...
            Workbook._resId = AddRelationship(OpenXMLRelationTypes.xl_workbook______, Workbook._xmlTarget);

            contentType.Overrides.Add(new ContentTypeOverrideItem
            {
                PartName = "/" + Workbook._xmlTarget,
        ...
        internal Stylesheet CreateStyles()
        {
            ...
            Workbook.AddRelationship(OpenXMLRelationTypes.styles___________, "styles.xml");

            contentType.Overrides.Add(new ContentTypeOverrideItem
            {
                PartName = "/" + Stylesheet._xmlTarget,
        ...
        internal SharedStrings CreateSharedStrings()
        {
            ...
            Workbook.AddRelationship(OpenXMLRelationTypes.shared_strings___, "sharedStrings.xml");

            contentType.Overrides.Add(new ContentTypeOverrideItem
            {
                PartName = "/" + SharedStrings._xmlTarget,
		...
		internal IO.OpenXML.Schema.Drawing CreateDrawing(Schema.Worksheet sheet)
		{
			...
			string resId =
 sheet.AddRelationship(OpenXMLRelationTypes.drawing__________, "../drawings/" + drawingFileName);
        ...
        internal void CreateCoreProperties()
        {
            ...
            coreProp._resId = AddRelationship(OpenXMLRelationTypes.docProps_core____, coreProp._xmlTarget);

            contentType.Overrides.Add(new ContentTypeOverrideItem
            {
                PartName = "/" + coreProp._xmlTarget,
        ...
        internal void CreateAppProperties()
        {
            ...
            appProp._resId = AddRelationship(OpenXMLRelationTypes.docProps_app_____, appProp._xmlTarget);

            contentType.Overrides.Add(new ContentTypeOverrideItem
            {
                PartName = "/" + appProp._xmlTarget,
        ...
        private void WriteFile<T>(string path, T obj)
        {
            var stream = new MemoryStream();
            {
                ...
                var ctEntry = zipArchive.AddFile(path, stream);
        ...
        {
            if (obj._relationFile != null) WriteFile(obj._relationFile._xmlTarget, obj._relationFile);
        ...
        public void Flush()
        {
            ...
            // .rels
            WriteFile(_relationFile._xmlTarget, _relationFile);

            #region Workbook

            WriteOpenXMLFile(Workbook);
            ...
            {
                SharedStrings.count = SharedStrings.items.Count.ToString();
                SharedStrings.uniqueCount = SharedStrings.items.Count.ToString();

                WriteFile(SharedStrings._xmlTarget, SharedStrings);
            }
            ...
            WriteFile(Stylesheet._xmlTarget, Stylesheet);
...
#region Floating Images
            ...
            if (coreProp != null) WriteFile(coreProp._xmlTarget, coreProp);

            #endregion // CoreProperties

            #region AppProperties

            if (appProp != null) WriteFile(appProp._xmlTarget, appProp);
...
namespace unvell.ReoGrid.IO.OpenXML.Schema
{
    ...
    partial class Workbook
    {
        ...
        internal Worksheet CreateWorksheet(string sheetName)
        {
            if (sheets == null) sheets = new List<WorkbookSheet>();

            var sheetId = sheets.Count + 1;

            var sheetFileName = string.Format("sheet{0}.xml", sheetId);
            var rsTarget = "worksheets/" + sheetFileName;
            ...
            sheetIndex.resId = AddRelationship(OpenXMLRelationTypes.worksheets_sheet_, rsTarget);
            sheets.Add(sheetIndex);

            _doc.contentType.Overrides.Add(new ContentTypeOverrideItem
            {
    ...
    partial class OpenXMLFile
    {
        internal string GetAvailableRelationId()
        {
            ...
            var index = _relationFile.relations.Count + 1;
            string rId = null;

            while (_relationFile.relations.Any(s =>
                       s.id.Equals(rId = "rId" + index, StringComparison.CurrentCultureIgnoreCase))) index++;
        ...
        {
            if (_relationFile == null) _relationFile = new Relationships(_rsTarget);

            if (_relationFile.relations == null) _relationFile.relations = new List<Relationship>();

            var rid = GetAvailableRelationId();

            _relationFile.relations.Add(new Relationship { id = rid, type = type, target = target });
