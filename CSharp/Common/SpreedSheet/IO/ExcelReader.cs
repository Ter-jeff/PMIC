...
using RGWorkbook = SpreedSheet.Core.Workbook.IWorkbook;
...
namespace unvell.ReoGrid.IO.OpenXML
{
    ...
    internal sealed class ExcelReader
    {
        private const NumberStyles
            numberStyle =
                NumberStyles
                    .Any; //System.Globalization.NumberStyles.Float | System.Globalization.NumberStyles.AllowThousands;
        ...
        public static void ReadStream(RGWorkbook rgWorkbook, Stream stream)
        {
            ...
            if (document.Workbook.sheets.Count == 0)
                throw new ReoGridLoadException("Specified Excel file does not contain any valid sheets.");
            ...
            {
                if (string.IsNullOrEmpty(definedName.name)) continue;

                ReadNamedRange(rgWorkbook, document.Workbook, definedName);
            }

            // load all worksheets
            foreach (var sheet in document.Workbook.sheets) LoadWorksheet(rgWorkbook, document, sheet);

#if DEBUG
            sw.Stop();
            var ms = sw.ElapsedMilliseconds;
            if (ms > 20) Logger.Log("file format", "reading excel format takes " + ms + " ms.");
#endif // DEBUG
        }
        ...
        private static void ReadNamedRange(RGWorkbook rgBook, Schema.Workbook workbook, DefinedName defName)
        {
            ...
            if (index > 0)
            {
                var space = name.Substring(0, index);
        ...
        {
            var rgSheet = rgWorkbook.GetWorksheetByName(sheetIndex.name);

            var sheet = doc.LoadRelationResourceById<Schema.Worksheet>(doc.Workbook, sheetIndex.resId);

            const float
                fixedCharWidth =
                    7.0f; //ResourcePoolManager.Instance.GetFont("Arial", 10f, System.Drawing.FontStyle.Regular).SizeInPoints;

            #region SheetView

            var sheetView = sheet.sheetViews.FirstOrDefault();

            if (sheetView != null)
            {
                if (sheetView.showGridLines != null)
                    rgSheet.SetSettings(WorksheetSettings.View_ShowGridLine,
                        OpenXMLUtility.IsTrue(sheetView.showGridLines));

                if (sheetView.showRowColHeaders != null)
                    rgSheet.SetSettings(WorksheetSettings.View_ShowHeaders,
                        OpenXMLUtility.IsTrue(sheetView.showRowColHeaders));

                if (sheetView.zoomScale != null)
                {
                    double zoom = 100;
                    if (double.TryParse(sheetView.zoomScale, numberStyle, CultureInfo.InvariantCulture, out zoom))
            ...
            var defaultRowHeight = rgSheet.defaultRowHeight;
            ...
            if (sheet.sheetFormatProperty != null)
            {
                ...
                if (sheet.sheetFormatProperty.defaultColumnWidth != null)
                {
                    ...
                    {
                        var pixelWidth = (ushort)Math.Truncate(
                            (256 * defColumnWidth + Math.Truncate(128 / fixedCharWidth)) / 256 * fixedCharWidth);
            ...
            var sheetRowCount = sheet.rows.Count;
            var sheetColCount = sheet.cols.Count;

            if (sheet.dimension != null && !string.IsNullOrEmpty(sheet.dimension.address))
            {
                var contentRange = new RangePosition(sheet.dimension.address);

                sheetRowCount = Math.Max(sheetRowCount, contentRange.EndRow + 1);
                sheetColCount = Math.Max(sheetColCount, contentRange.EndCol + 1);
            }

            if (rgSheet.RowCount < sheetRowCount) rgSheet.Rows = sheetRowCount;

            if (rgSheet.ColumnCount < sheetColCount) rgSheet.Columns = sheetColCount;
            ...
            if (sheet.cols != null)
                foreach (var col in sheet.cols)
                {
                    ...
                    var pixelWidth = (ushort)Math.Truncate((256 * col.width + Math.Truncate(128 / fixedCharWidth)) /
                        256 * fixedCharWidth);

                    var startCol = col.min - 1;
                    var count = col.max - col.min + 1;
            ...
            // data
            var sharedStringTable = doc.ReadSharedStringTable();

            var defaultFont = fonts.list.ElementAtOrDefault(0);
            if (defaultFont != null) SetStyleFont(doc, rgSheet.RootStyle, defaultFont);

#if DEBUG
            var sw = Stopwatch.StartNew();
            Logger.Log("excel format", "begin iterating cells...");

            var swBorder = new Stopwatch();
            var swStyle = new Stopwatch();
            var swValue = new Stopwatch();
            var swFormula = new Stopwatch();
            var swRowStyle = new Stopwatch();
            var swRowHeight = new Stopwatch();
            var swDrawing = new Stopwatch();

            var cellCount = 0;
#endif

            var sharedFormulas = new Dictionary<int, SharedFormulaInfo>();

            var rowTop = rgSheet.GetRowHeader(0).Top;
            var lastRowIndex = -1;
            ...
            foreach (var row in sheet.rows)
            {
                ...
                if (rowIndex - lastRowIndex > 1)
                {
                    ...
                    for (var i = lastRowIndex + 1; i < rowIndex; i++)
                    {
                ...
                // row height
                double rowHeight = 0;
                var isHidden = OpenXMLUtility.IsTrue(row.hidden);

                RowHeader rowHeader;

                if ( //row.customHeight == "1"
                    //&& 
                    !string.IsNullOrEmpty(row.height) && double.TryParse(row.height, numberStyle,
                        CultureInfo.InvariantCulture, out rowHeight))
                {
                    rowHeader = rgSheet.GetRowHeader(rowIndex);
                    var height = (ushort)Math.Round(rowHeight * dpi / 72f);
                ...
                if (!string.IsNullOrEmpty(row.styleIndex) && int.TryParse(row.styleIndex, out styleIndex))
                {
                    ...
                    if (style != null)
                    {
                        if (!style._preprocessed) PreprocessCellStyle(doc, rgSheet, style, fonts, fills);
                            ...
                            !string.IsNullOrEmpty(style.borderId))
                        {
                                ...
                                && id >= 0 && id < borders.Count)
                            {
                                ...
                                if (border != null)
                                {
                                    if (!border._preprocessed) PreprocessCellBorders(doc, border);

                                    if (border._hasTop)
                                    {
                                        for (var c = 0; c < rgSheet.Columns; c++)
                                        {
                                            var hb = rgSheet.GetHBorder(rowIndex, c);
                                            hb.Pos |= HBorderOwnerPosition.Top;
                                    ...
                                    if (border._hasBottom)
                                    {
                                        for (var c = 0; c < rgSheet.Columns; c++)
                                        {
                                            ...
                                            hb.Pos |= HBorderOwnerPosition.Bottom;
                                    ...
                                    if (border._hasLeft)
                                    {
                                        for (var c = 0; c < rgSheet.Columns; c++)
                                        {
                                            ...
                                            vb.Pos |= VBorderOwnerPosition.Left;
                                    ...
                                    if (border._hasRight)
                                    {
                                        for (var c = 0; c < rgSheet.Columns; c++)
                                        {
                                            ...
                                            vb.Pos |= VBorderOwnerPosition.Right;
                ...
                {
                    var pos = new CellPosition(cell.address);
                    var rgCell = rgSheet.cells[pos.Row, pos.Col];
                    ...
                    if (!string.IsNullOrEmpty(cell.styleIndex) && int.TryParse(cell.styleIndex, out styleIndex))
                    {
                        ...
                        if (style != null)
                        {
                            ...
                            if (style != null
                                ...
                                && borderId < borders.Count //style.applyBorder == "1" || 
                                && borderId > 0)
                            {
...
#if DRAWING
										rgCell.InnerData =
 CreateRichTextFromRuns(doc, new Paragraph[] { new Paragraph { runs = ssitem.runs } });
#endif // DRAWING
                                    }
                                }
                            }
                        ...
                        {
                            decimal dv;
                            if (decimal.TryParse(cell.value.val, numberStyle, CultureInfo.InvariantCulture, out dv))
                                rgCell.InnerData = dv;
                            else
                    ...
                    if (rgCell.DataFormat == CellDataFormatFlag.DateTime)
                        rgCell.Data = DataFormatterManager.Instance.DataFormatters[CellDataFormatFlag.DateTime]
                            .FormatCell(rgCell);
                    else
						...
						if (formula.type == "shared")
						{
							if (int.TryParse(formula.sharedIndex, out var sharedIndex))
							{
								if (!string.IsNullOrEmpty(formula.val))
								{
									...
									sharedFormulas[sharedIndex] = new SharedFormulaInfo { pos = pos, formula =
 cellFormula };
								}
								else
								{
            ...
            //int offset = 0;
            for (var i = lastRowIndex + 1; i < rgSheet.Rows; i++)
            {
            ...
            if (sheet.mergeCells != null)
                foreach (var mcell in sheet.mergeCells)
            ...
            var lastBorder = RangeBorderStyle.Empty;
            var lastHPos = HBorderOwnerPosition.None;
            var borderSpans = 0;

            for (var r = maxHBorderRow; r >= 0; r--)
            {
                lastBorder = RangeBorderStyle.Empty;
                lastHPos = HBorderOwnerPosition.None;
                borderSpans = 1;

                for (var c = maxHBorderCol; c >= 0; c--)
                {
            ...
            lastBorder = RangeBorderStyle.Empty;
            var lastVPos = VBorderOwnerPosition.None;
            borderSpans = 1;

            // vertical normalize
            for (var c = maxVBorderCol; c >= 0; c--)
            {
                lastBorder = RangeBorderStyle.Empty;
                lastVPos = VBorderOwnerPosition.None;
                borderSpans = 1;

                for (var r = maxVBorderRow; r >= 0; r--)
                {
            ...
            if (sheetView != null && sheetView.pane != null)
            {
                ...
                if (sheetView.pane.ySplit != null) int.TryParse(sheetView.pane.ySplit, out freezeRow);
                if (sheetView.pane.xSplit != null) int.TryParse(sheetView.pane.xSplit, out freezeCol);

                if (freezeRow > 0 || freezeCol > 0) rgSheet.FreezeToCell(new CellPosition(freezeRow, freezeCol));
            }
            ...
            Debug.WriteLine("Row  Height  : {0} ms.", swRowHeight.ElapsedMilliseconds);
            Debug.WriteLine("Row  Style   : {0} ms.", swRowStyle.ElapsedMilliseconds);
            Debug.WriteLine("Cell Style   : {0} ms.", swStyle.ElapsedMilliseconds);
            Debug.WriteLine("Cell Border  : {0} ms.", swBorder.ElapsedMilliseconds);
            Debug.WriteLine("Cell Value   : {0} ms.", swValue.ElapsedMilliseconds);
            Debug.WriteLine("Cell Formula : {0} ms.", swFormula.ElapsedMilliseconds);
            Debug.WriteLine("Drawing      : {0} ms.", swDrawing.ElapsedMilliseconds);
        ...
        private static void SetStyleFont(Document doc, WorksheetRangeStyle styleset, Font font)
        {
        ...
        {
            var styleset = new WorksheetRangeStyle(rgSheet.RootStyle);
            var tempColor = new SolidColor();
                ...
                !string.IsNullOrEmpty(style.fontId))
            {
                ...
                if (font != null) SetStyleFont(doc, styleset, font);
            }
                ...
                !string.IsNullOrEmpty(style.fillId))
            {
                ...
                if (fill != null && fill.patternFill != null && fill.patternFill.patternType != null)
                    switch (fill.patternFill.patternType)
                    {
                        ...
                        case "darkTrellis":
                            var foreColor = SolidColor.Transparent;
                ...
                style.alignment != null)
            {
                ...
                {
                    var halign = GridHorAlign.Left;
                    switch (style.alignment.horizontal)
                    {
                        default:
                        case "left":
                            halign = GridHorAlign.Left;
                            break;
                ...
                //{
                var valign = GridVerAlign.General;
                ...
                {
                    ushort indent;
                    if (ushort.TryParse(style.alignment.indent, out indent)) styleset.Indent = indent;
                }
            }

            #endregion // Alignment

            if (styleset.Flag != PlainStyleFlag.None) style._cachedStyleSet = styleset;
                ...
                && themeIndex >= 0)
            {
                ...
                if (theme != null)
                        ...
                        && theme.elements.clrScheme != null)
                    {
                        ...
                        {
                            case 0:
                                rgColor = SolidColor.White;
                                break; // doc.ConvertFromCompColor(theme.elements.clrScheme.dk1); break;
                            case 1:
                                rgColor = SolidColor.Black;
                                break; // doc.ConvertFromCompColor(theme.elements.clrScheme.lt1); break;
                            case 2:
                                rgColor = doc.ConvertFromCompColor(theme.elements.clrScheme.dk2);
                                break;
                            case 3:
                                rgColor = doc.ConvertFromCompColor(theme.elements.clrScheme.lt2);
                                break;
                            case 4:
                                rgColor = doc.ConvertFromCompColor(theme.elements.clrScheme.accent1);
                                break;
                            case 5:
                                rgColor = doc.ConvertFromCompColor(theme.elements.clrScheme.accent2);
                                break;
                            case 6:
                                rgColor = doc.ConvertFromCompColor(theme.elements.clrScheme.accent3);
                                break;
                            case 7:
                                rgColor = doc.ConvertFromCompColor(theme.elements.clrScheme.accent4);
                                break;
                            case 8:
                                rgColor = doc.ConvertFromCompColor(theme.elements.clrScheme.accent5);
                                break;
                            case 9:
                                rgColor = doc.ConvertFromCompColor(theme.elements.clrScheme.accent6);
                                break;
                            case 10:
                                rgColor = doc.ConvertFromCompColor(theme.elements.clrScheme.hlink);
                                break;
                            case 11:
                                rgColor = doc.ConvertFromCompColor(theme.elements.clrScheme.folHlink);
                                break;
                            default:
                                rgColor = SolidColor.Black;
                                break;
                        ...
                        if (double.TryParse(color.tint, numberStyle, CultureInfo.InvariantCulture, out tint))
                        {
                            var hlsColor = ColorUtility.RGBToHSL(rgColor);
                            hlsColor.L = ColorUtility.CalculateFinalLumValue((float)tint, hlsColor.L * 255f) / 255f;
            ...
            if (!string.IsNullOrEmpty(color.indexed)
                ...
                switch (colorIndex)
                {
                    ...
                    default:
                    {
                        ...
                        if (colorIndex < IndexedColorTable.colors.Length)
                        {
        ...
        {
            var posStyle = new RangeBorderStyle();
        ...
        private static bool ConvertFromExcelBorder(Document doc, SideBorder sideBorder, ref RangeBorderStyle rgStyle)
        {
            if (sideBorder != null && !string.IsNullOrEmpty(sideBorder.style))
            {
                ...
                if (ConvertFromIndexedColor(doc, sideBorder.color, ref color))
                {
                    rgStyle.Color = color;
                    return true;
                }

                if (OpenXMLUtility.IsTrue(sideBorder.color.auto))
                {
                    rgStyle.Color = SolidColor.Black;
                    return true;
                }
        ...
        private static readonly Regex currencyFormatRegex =
            new Regex(@"([^\\\s]*)\\?(\s*)\[\$([^(\-|\])]+)-?[^\]]*\]\\?(\s*)([^\\\s]*)", RegexOptions.Compiled);

        private static NumberDataFormatter.INumberFormatArgs ReadNumberFormatArgs(string pattern,
            NumberDataFormatter.INumberFormatArgs arg, CultureInfo culture)
        {
            ...
            if (pattern.StartsWith("\""))
            {
                ...
                var prefix = pattern.Substring(1, index - 1);
            ...
            var decimalSeparatorIndex = pattern.LastIndexOf(culture.NumberFormat.NumberDecimalSeparator, len - 1);
        ...
        private enum NumberFormatParseStatus
        {
            Segment,
            InString
        }

        private enum NumberFormatSegmentDefines
        {
            Positive = 0,
            Negative = 1,
            Zero = 2,
        ...
        private static CellDataFormatFlag SetRGSheetDataFormat(RGWorksheet rgSheet, Cell cell, int formatId,
            Stylesheet styles)
        {
            var flag = CellDataFormatFlag.General;

            if (BuiltinNumberFormats.SetFromExcelBuiltinFormat(rgSheet, cell, formatId, out flag)) return flag;

            if (styles.numberFormats != null)
            {
                var numFormat = styles.numberFormats.list.FirstOrDefault(nf => nf.formatId == formatId);

                if (numFormat != null)
                {
                    var patterns = numFormat.formatCode.Split(';');
                    ...
                    if (patterns != null && patterns.Length > 0)
                    {
                        ...
                        else if ((currencyMatch = currencyFormatRegex.Match(pattern)).Success)
                        {
                            ...
                            {
                                if (currencyMatch.Groups[3].Success) carg.PostfixSymbol = currencyMatch.Groups[3].Value;

                                if (currencyMatch.Groups[2].Length > 0)
                                    carg.PostfixSymbol = currencyMatch.Groups[2].Value + carg.PostfixSymbol;

                                carg = (CurrencyDataFormatter.CurrencyFormatArgs)ReadNumberFormatArgs(
                                    currencyMatch.Groups[1].Value, carg, CultureInfo.InvariantCulture);
                            }
                            else if (currencyMatch.Groups[5].Length > 0)
                            {
                                if (currencyMatch.Groups[3].Success) carg.PrefixSymbol = currencyMatch.Groups[3].Value;

                                if (currencyMatch.Groups[4].Length > 0)
                                    carg.PrefixSymbol += currencyMatch.Groups[4].Value;

                                carg = (CurrencyDataFormatter.CurrencyFormatArgs)ReadNumberFormatArgs(
                                    currencyMatch.Groups[5].Value, carg, CultureInfo.InvariantCulture);
                            }
                        ...
                        else
                        {
                            ...
                            arg = ReadNumberFormatArgs(patterns.Length > 1 ? patterns[1] : patterns[0],
                                new NumberDataFormatter.NumberFormatArgs(), CultureInfo.InvariantCulture);
                        }
			...
			if (blipFill != null)
			{
				...
				if (blip != null && !string.IsNullOrEmpty(blip.embedId) && drawingFile._relationFile != null)
				{
					...
					if (stream != null)
					{
						...
						try
						{
							System.Windows.Media.Imaging.BitmapImage biImg =
...
#region LoadShape
		...
		private static Drawing.Shapes.Line LoadLine(ShapeProperty prop)
		{
			...
			if (xfrm != null)
			{
				...
				Rectangle bounds =
 new Rectangle(MeasureToolkit.EMUToPixel(xfrm.offset.x, dpi), MeasureToolkit.EMUToPixel(xfrm.offset.y, dpi),
			...
			if (graphic != null && graphic.data != null)
			{
				switch (graphic.data.uri)
				{
					case OpenXMLNamespaces.Chart________:
						...
						{
							var xmlChart =
 doc.LoadRelationResourceById<Schema.ChartSpace>(drawingFile, graphic.data.chart.id);
							if (xmlChart != null)
							{
			...
			foreach (var p in paragraphs)
			{
				...
				if (p.property != null)
				{
					if (p.property.align != null)
					{
						...
						{
							case "ctr":
								rt.SetStyles(halign: GridHorAlign.Center);
								break;
						}
					}
				}
...
#region Run Property
    ...
    internal class SharedFormulaInfo
    {
    ...
    {
        public static bool SetFromExcelBuiltinFormat(RGWorksheet rgSheet, Cell cell, int formatId,
            out CellDataFormatFlag dataFormatFlag)
        {
            ...
            switch (formatId)
            {
                ...
                case 22:
                    ...
                    arg = new DateTimeDataFormatter.DateTimeFormatArgs
                    {
                        ...
                        Format = DateTimeFormat.ResourceManager.GetString(formatId.ToString(), currentCulture)
                    };
                    break;
    ...
    internal partial class Document
    {
        ...
        {
            var zip = MZipArchiveFactory.OpenOnStream(stream);
        ...
        {
            var obj = LoadObjectFromPath<T>(path, name);
        ...
        {
            var finalPath = entryFile._path + "_rels/" + name + ".rels";

            if (zipArchive.IsFileExist(finalPath))
                entryFile._relationFile = LoadObjectFromPath<Relationships>(finalPath, null);
        }
        ...
        internal T LoadRelationResourceById<T>(OpenXMLFile entryFile, string id) where T : class
        {
            ...
            return LoadObjectFromPath<T>(entryFile._path, relation.target);
        }

        internal T LoadRelationResourceByType<T>(OpenXMLFile entryFile, string typeNamespace) where T : class
        {
            var relation = Workbook._relationFile.relations.FirstOrDefault(r => r.type == typeNamespace);
            return relation == null ? null : LoadObjectFromPath<T>(entryFile._path, relation.target);
        }
        ...
        internal T LoadObjectFromPath<T>(string path, string name) where T : class
        {
            ...
            var entry = zipArchive.GetFile(finalPath);

            if (entry == null) throw new ExcelFormatException("Resource entry cannot be found: " + path);

            using (var stream = entry.GetStream())
            {
                if (stream == null) throw new ExcelFormatException("Resource stream cannot be found: " + path);

                var obj = XMLHelper.LoadXML<T>(stream);
                ...
                if (obj is OpenXMLFile)
                {
                    ...
                    LoadRelationShipsFile(entryFile, objectName);
                }
        ...
        {
            var entry = zipArchive.GetFile(path);
        ...
        {
            if (zipArchive.IsFileExist(Workbook._path + sharedStrings_xml_filename))
                return LoadEntryFile<SharedStrings>(Workbook._path, sharedStrings_xml_filename);
            return null;
        }
        ...
        public Theme Themesheet
        {
            get
            {
                if (themesheet == null && zipArchive != null)
                    themesheet = LoadRelationResourceByType<Theme>(Workbook, OpenXMLRelationTypes.theme____________);

                return themesheet;
            }
            set { themesheet = value; }
        }
        ...
        {
            if (compColor._solidColor.A > 0) return compColor._solidColor;
            ...
            if (compColor.schemeColor != null)
            {
                ...
                {
                    case "dk1":
                        color = ConvertFromCompColor(theme.elements.clrScheme.dk1);
                        break;
                    case "lt1":
                        color = ConvertFromCompColor(theme.elements.clrScheme.lt1);
                        break;
                    case "dk2":
                        color = ConvertFromCompColor(theme.elements.clrScheme.dk2);
                        break;
                    case "lt2":
                        color = ConvertFromCompColor(theme.elements.clrScheme.lt2);
                        break;
                    case "accent1":
                        color = ConvertFromCompColor(theme.elements.clrScheme.accent1);
                        break;
                    case "accent2":
                        color = ConvertFromCompColor(theme.elements.clrScheme.accent2);
                        break;
                    case "accent3":
                        color = ConvertFromCompColor(theme.elements.clrScheme.accent3);
                        break;
                    case "accent4":
                        color = ConvertFromCompColor(theme.elements.clrScheme.accent4);
                        break;
                    case "accent5":
                        color = ConvertFromCompColor(theme.elements.clrScheme.accent5);
                        break;
                    case "accent6":
                        color = ConvertFromCompColor(theme.elements.clrScheme.accent6);
                        break;
                    case "hlink":
                        color = ConvertFromCompColor(theme.elements.clrScheme.hlink);
                        break;
                    case "folHlink":
                        color = ConvertFromCompColor(theme.elements.clrScheme.folHlink);
                        break;
                    case "phClr":
                        color = ConvertFromCompColor(overrideColor);
                        break;
                }

                if (overrideColor != null)
                {
                    var hlsColor = ColorUtility.RGBToHSL(color);

                    var compColorVal = overrideColor.schemeColor;

                    if (compColorVal.shade != null)
                        hlsColor.L = ColorUtility.CalculateFinalLumValue(-(float)compColorVal.shade.value / 100000f,
                            hlsColor.L * 255f) / 255f;

                    if (compColorVal.tint != null)
                        hlsColor.L =
                            ColorUtility.CalculateFinalLumValue(compColorVal.tint.value / 100000f, hlsColor.L * 255f) /
                            255f;

                    if (compColorVal.lumMod != null) hlsColor.L *= compColorVal.lumMod.value / 100000f;

                    if (compColorVal.lumOff != null) hlsColor.L += compColorVal.lumOff.value / 100000f;

                    if (compColorVal.satMod != null) hlsColor.S *= compColorVal.satMod.value / 100000f;
    ...
    internal class ExcelFormatException : ReoGridException
    {
