...
namespace unvell.ReoGrid
{
    partial class Worksheet
    {
        ...
        public ReoGridRangeCollection Ranges
        {
            get
            {
                if (rangeCollection == null) rangeCollection = new ReoGridRangeCollection(this);
        ...
        public class ReoGridRangeCollection
        {
            ...
            {
                get { return new ReferenceRange(worksheet, address); }
            }
            ...
            public ReferenceRange this[int row, int col, int rows, int cols]
            {
                get
                {
                    return new ReferenceRange(worksheet, worksheet.FixRange(new RangePosition(row, col, rows, cols)));
                }
            }
            ...
            public ReferenceRange this[CellPosition startPos, CellPosition endPos]
            {
                get
                {
                    return new ReferenceRange(worksheet,
                        worksheet.CreateAndGetCell(startPos), worksheet.CreateAndGetCell(endPos));
                }
            }
            ...
            public ReferenceRange this[RangePosition range]
            {
                get
                {
                    if (range.IsEmpty) throw new ArgumentException("range position is empty", nameof(range));

                    return new ReferenceRange(worksheet, worksheet.FixRange(range));
                }
            }
        }
        ...
        public object[,] GetRangeData(RangePosition range)
        {
            ...
            var data = new object[fixedRange.Rows, fixedRange.Cols];
        ...
        public void DeleteRangeData(string addressOrName)
        {
            ...
            {
                DeleteRangeData(new RangePosition(addressOrName), true);
            }
            else
            {
                NamedRange refRange;
                if (registeredNamedRanges.TryGetValue(addressOrName, out refRange))
                    DeleteRangeData(refRange, true);
                else
        ...
        /// </summary>
        /// <param name="range">Range to remove data array</param>
        /// <param name="checkReadonly">
        ///     if this flag is set to true, the value from
        ///     readonly cells will not be deleted
        /// </param>
        public void DeleteRangeData(RangePosition range, bool checkReadonly = false)
        {
            var fixedRange = FixRange(range);

            var maxcol = range.Col;
            ...
            IterateCells(fixedRange, (row, col, cell) =>
            {
                ...
                if (!checkReadonly || !cell.IsReadOnly)
                {
                    ...
                    cell.RenderScaleFactor = renderScaleFactor;
        ...
        public void SetRangeData(string addressOrName, object data)
        {
            if (RangePosition.IsValidAddress(addressOrName))
            {
                SetRangeData(new RangePosition(addressOrName), data);
            }
            else
            {
                NamedRange refRange;
                if (registeredNamedRanges.TryGetValue(addressOrName, out refRange))
                    SetRangeData(refRange, data);
                else
                    throw new InvalidAddressException(addressOrName);
            }
        }
        ...
        /// <param name="checkReadonly">
        ///     Ignore to update read-only cells if this flag is true;
        ///     Set this flag as fasle to update all cells in specified range. (Default is true)
        /// </param>
        public void SetRangeData(RangePosition range, object data, bool checkReadonly)
        {
            ...
            if (data is Array)
            {
                ...
                if (arr.Rank == 2)
                {
                    var dataRows = arr.GetLength(0);
                    var dataCols = arr.GetLength(1);

                    var maxRows = Math.Max(dataRows, range.Rows);
                    var maxCols = Math.Max(dataCols, range.Cols);

                    for (var r = 0; r < maxRows && r < dataRows; r++)
                    for (int cc = 0, index = 0; cc < maxCols && index < dataCols; index++)
                    {
                        var targetRow = range.Row + r;
                        var targetCol = range.Col + cc;

                        if (targetRow < rows.Count && targetCol < cols.Count)
                        {
                            var cell = CreateAndGetCell(targetRow, targetCol);

                            if (!checkReadonly || !cell.IsReadOnly) SetSingleCellData(cell, arr.GetValue(r, index));
                ...
                {
                    var nestedList = arr.Length > 0 && arr.GetValue(0) is IEnumerable<object>;
                        ...
                        for (int r = range.Row, i = 0; r <= range.EndRow && i < arr.Length; r++, i++)
                        {
                            ...
                            if (elements != null)
                                foreach (var ele in elements)
                                {
                                    SetCellData(r, c, ele);
                                    c++;
                                    if (c > range.EndCol || c >= cols.Count) break;
                                }
                        }
                    }
                    else
        ...
        internal void RaiseRangeDataChangedEvent(RangePosition range)
        {
            ...
            {
                if (RangeDataChanged != null) RangeDataChanged(this, new RangeEventArgs(range));
        ...
        public CellPosition FixPos(CellPosition pos)
        {
            ...
            if (pos.Row > rows.Count - 1) pos.Row = rows.Count - 1;
            if (pos.Col > cols.Count - 1) pos.Col = cols.Count - 1;
            return pos;
        }
        ...
        public RangePosition FixRange(RangePosition range)
        {
            ...
            if (range.Rows == -1) range.EndRow = rows.Count;
            if (range.Cols == -1) range.EndCol = cols.Count;
            if (range.EndRow > rows.Count - 1 || range.Rows == -1) range.EndRow = rows.Count - 1;
            if (range.EndCol > cols.Count - 1 || range.Cols == -1) range.EndCol = cols.Count - 1;
            return range;
        }
        ...
        {
            var foundReadonlyCell = false;

            IterateCells(range, (r, c, cell) =>
            {
                if (cell.IsReadOnly)
                {
        ...
        {
            ScrollToCell(new CellPosition(address));
        }
        ...
        {
            ScrollToRange(new RangePosition(pos), pos);
        }
        ...
        {
            ScrollToRange(FixRange(new RangePosition(row, col, 1, 1)));
        }
        ...
        public void ScrollToRange(string addressOrName)
        {
            if (RangePosition.IsValidAddress(addressOrName))
            {
                ScrollToRange(new RangePosition(addressOrName));
            }
            else
            {
                NamedRange refRange;
                if (registeredNamedRanges.TryGetValue(addressOrName, out refRange))
                    ScrollToRange(refRange);
                else
                    throw new InvalidAddressException(addressOrName);
            }
        }
        ...
        {
            ScrollToRange(new RangePosition(row, col, rows, cols));
        }
        ...
        {
            ScrollToRange(range, range.EndPos);
        }
        ...
        public void ScrollToRange(RangePosition range, CellPosition basePos)
        {
            ...
            if (svc != null) svc.ScrollToRange(FixRange(range), basePos);
        }
        ...
        public void MoveRange(string fromRangeAddress, string toRangeAddress)
        {
            ...
            else if (TryGetNamedRangePosition(fromRangeAddress, out fromRange))
            {
                // nothing to do
            }
            else
            ...
            else if (TryGetNamedRangePosition(toRangeAddress, out toRange))
            {
                // nothing to do
            }
            else
        ...
        /// <exception cref="RangeIntersectionException">
        ///     Range to be moved or copied to that contains a part of another merged
        ///     cell.
        /// </exception>
        ...
        /// </exception>
        public void MoveRange(RangePosition fromRange, RangePosition toRange,
            PartialGridCopyFlag flags = PartialGridCopyFlag.All)
        {
        ...
        /// </summary>
        /// <remarks>
        ///     The range copied from cannot be a part of another range,
        ///     <para>fromRange</para>
        ///     contains a part of
        ...
        /// <exception cref="CellDataReadonlyException">Throw when current worksheet is read-only.</exception>
        /// <exception cref="RangeIntersectionException">Range to be moved or copied contains a part of another merged cell.</exception>
        /// <exception cref="RangeContainsReadonlyCellsException">
        ///     Thrown when specified target range contains any read-only cell
        ...
        /// <param name="flags">
        ///     Content flags that specifiy the types of content to be copied. (Value of this flag can be data,
        ///     borders, styles and etc.)
        /// </param>
        /// <exception cref="CellDataReadonlyException">Throw when current worksheet is read-only.</exception>
        /// <exception cref="RangeIntersectionException">
        ///     Range to be moved or copied to that contains a part of another merged
        ...
        ///     Operation will be cancelled if
        ///     <para>fromRange</para>
        ///     is same as
        ///     <para>toRange</para>
        /// </remarks>
        ...
        ///     Determines that whether or not to remove content from original range. Set true to move range;
        ///     set false to copy range.
        /// </param>
        /// <exception cref="CellDataReadonlyException">Throw when current worksheet is read-only.</exception>
        /// <exception cref="RangeIntersectionException">Range to be moved or copied contains a part of another merged cell.</exception>
        /// <exception cref="RangeContainsReadonlyCellsException">
        ...
        ///     that cannot be overwritten.
        /// </exception>
        internal void CopyRange(RangePosition fromRange, RangePosition toRange,
            PartialGridCopyFlag flags = PartialGridCopyFlag.All,
            bool moveRange = false)
        {
            if (HasSettings(WorksheetSettings.Edit_Readonly)) throw new CellDataReadonlyException(fromRange.StartPos);
            ...
            var intersectedRange = CheckIntersectedMergingRange(fromRange);

            if (intersectedRange != RangePosition.Empty) throw new RangeIntersectionException(fromRange);

            intersectedRange = CheckIntersectedMergingRange(toRange);

            if (intersectedRange != RangePosition.Empty) throw new RangeIntersectionException(toRange);

            if (CheckRangeReadonly(fromRange)) throw new RangeContainsReadonlyCellsException(fromRange);

            if (CheckRangeReadonly(toRange)) throw new RangeContainsReadonlyCellsException(fromRange);

            var bcmrearg = new BeforeCopyOrMoveRangeEventArgs(fromRange, toRange);

            if (moveRange)
                BeforeRangeMove?.Invoke(this, bcmrearg);
            else
                BeforeRangeCopy?.Invoke(this, bcmrearg);
        ...
        public void ClearRangeContent(string addressOrName, CellElementFlag flags)
        {
            ...
            else if (TryGetNamedRange(addressOrName, out namedRange))
        ...
        public void ClearRangeContent(RangePosition range, CellElementFlag flags, bool checkReadonly = true)
        {
            ...
            var deleteData = (flags & CellElementFlag.Data) == CellElementFlag.Data;
            var deleteFormula = (flags & CellElementFlag.Formula) == CellElementFlag.Formula;
            var deleteDataFormat = (flags & CellElementFlag.DataFormat) == CellElementFlag.DataFormat;
            var deleteBody = (flags & CellElementFlag.Body) == CellElementFlag.Body;
            var deleteStyle = (flags & CellElementFlag.Style) == CellElementFlag.Style;
                ...
                if (cell != null)
                {
                    ...
                    if (!checkReadonly || !cell.IsReadOnly)
                    {
                        if (deleteData)
                        {
                            ...
                            if (cell.InnerStyle.HAlign == GridHorAlign.General)
            ...
            if (deleteStyle) RemoveRangeStyles(fixedRange, PlainStyleFlag.All);
            ...
            if ((flags & CellElementFlag.Border) == CellElementFlag.Border)
                RemoveRangeBorders(fixedRange, BorderPositions.All);
