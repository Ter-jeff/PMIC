...
namespace SpreedSheet.Core
{
    ...
    public struct CellPosition : ISheetAddress
    {
        ...
        public PositionProperty RowProperty
        {
            get
            {
                return (positionProperties & PositionAbsoluteBits.StartRow) == PositionAbsoluteBits.StartRow
            ...
            set
            {
                if (value == PositionProperty.Absolute)
                    positionProperties |= PositionAbsoluteBits.StartRow;
                else
                    positionProperties = (byte)(positionProperties & ~PositionAbsoluteBits.StartRow);
            }
        }
        ...
        public PositionProperty ColumnProperty
        {
            get
            {
                return (positionProperties & PositionAbsoluteBits.StartCol) == PositionAbsoluteBits.StartCol
                    ? PositionProperty.Absolute
                    : PositionProperty.Relative;
            }
            set
            {
                if (value == PositionProperty.Absolute)
                    positionProperties |= PositionAbsoluteBits.StartCol;
                else
                    positionProperties = (byte)(positionProperties & ~PositionAbsoluteBits.StartCol);
            }
        }
        ...
        {
            var m = RGUtility.CellReferenceRegex.Match(address);

            if (!m.Success) throw new ArgumentException("invalid address for cell: " + address, "id");

            row = 0;
            int.TryParse(m.Groups["row"].Value, out row);
            row--;
            col = RGUtility.GetNumberOfChar(m.Groups["col"].Value);

            positionProperties = 0;

            if (m.Groups["abs_row"].Success) positionProperties |= PositionAbsoluteBits.StartRow;

            if (m.Groups["abs_col"].Success) positionProperties |= PositionAbsoluteBits.StartCol;
        }
        ...
        {
            if (positionProperties == (PositionAbsoluteBits.StartRow | PositionAbsoluteBits.StartCol))
                return "$" + RGUtility.GetAlphaChar(col) + "$" + (row + 1);
            if (positionProperties == PositionAbsoluteBits.StartCol)
                return "$" + RGUtility.GetAlphaChar(col) + (row + 1);
            if (positionProperties == PositionAbsoluteBits.StartRow)
                return RGUtility.GetAlphaChar(col) + "$" + (row + 1);
            return RGUtility.GetAlphaChar(col) + (row + 1);
        }
        ...
        {
            return string.Format("${0}${1}", RGUtility.GetAlphaChar(col), row + 1);
        }
    ...
    public struct RangePosition : IRange, ISheetAddress
    {
        ...
        public CellPosition StartPos
        {
            ...
            set
            {
                row = value.Row;
                col = value.Col;
                StartRowProperty = value.RowProperty;
                StartColumnProperty = value.ColumnProperty;
            }
        }
        ...
        public CellPosition EndPos
        {
            get
            {
                return new CellPosition(rows == EntireRange.rows ? 1048576 : EndRow,
                    cols == EntireRange.cols ? 32768 : EndCol,
                    (byte)((positionProperties & (PositionAbsoluteBits.EndRow | PositionAbsoluteBits.EndCol)) >> 2));
            }
            set
            {
                EndRow = value.Row;
                EndCol = value.Col;
                EndRowProperty = value.RowProperty;
                EndColumnProperty = value.ColumnProperty;
            }
        }
        ...
        public PositionProperty EndRowProperty
        {
            get
            {
                return (positionProperties & PositionAbsoluteBits.EndRow) == PositionAbsoluteBits.EndRow
                    ? PositionProperty.Absolute
                    : PositionProperty.Relative;
            }
            set
            {
                if (value == PositionProperty.Absolute)
                    positionProperties |= PositionAbsoluteBits.EndRow;
                else
                    positionProperties = (byte)(positionProperties & ~PositionAbsoluteBits.EndRow);
            }
        }
        ...
        public PositionProperty EndColumnProperty
        {
            get
            {
                return (positionProperties & PositionAbsoluteBits.EndCol) == PositionAbsoluteBits.EndCol
                    ? PositionProperty.Absolute
                    : PositionProperty.Relative;
            }
            set
            {
                if (value == PositionProperty.Absolute)
                    positionProperties |= PositionAbsoluteBits.EndCol;
                else
                    positionProperties = (byte)(positionProperties & ~PositionAbsoluteBits.EndCol);
            }
        }
        ...
        public RangePosition(string address)
        {
            ...
            var m = RGUtility.RangeReferenceRegex.Match(address);

            if (!m.Success) m = RGUtility.SingleAbsoulteRangeRegex.Match(address);
            ...
            if (!m.Success && CellPosition.IsValidAddress(address))
            {
                ...
                StartRowProperty = pos.RowProperty;
                StartColumnProperty = pos.ColumnProperty;
                EndRowProperty = pos.RowProperty;
                EndColumnProperty = pos.ColumnProperty;
            }
            else
            {
                if (!m.Success
                ...
                if (fullCols)
                {
                    row = Math.Min(fromRow, toRow);
                    rows = Math.Max(fromRow, toRow) - row + 1;
                    col = 0;
                    cols = -1;
                }
                else if (fullRows)
                {
                    row = 0;
                    rows = -1;
                    col = Math.Min(fromCol, toCol);
                    cols = Math.Max(fromCol, toCol) - col + 1;
                }
                else
                {
                    row = Math.Min(fromRow, toRow);
                ...
                if (m.Groups["abs_from_row"].Success) positionProperties |= PositionAbsoluteBits.StartRow;

                if (m.Groups["abs_from_col"].Success) positionProperties |= PositionAbsoluteBits.StartCol;

                if (m.Groups["abs_to_row"].Success) positionProperties |= PositionAbsoluteBits.EndRow;

                if (m.Groups["abs_to_col"].Success) positionProperties |= PositionAbsoluteBits.EndCol;
            }
        }
        ...
        {
            return row <= range.row && col <= range.col && EndRow >= range.EndRow && EndCol >= range.EndCol;
        }
        ...
        {
            return row >= this.row && row <= EndRow;
        }
        ...
        {
            return col >= this.col && col <= EndCol;
        }
        ...
        public bool IntersectWith(RangePosition range)
        {
            ...
            return !(col + cols - 1 < range.col
                     || range.col + range.cols - 1 < col
                     || row + rows - 1 < range.row
                     || range.row + range.rows - 1 < row);
        }
        ...
        public string ToAddress()
        {
            if (rows <= -1 && cols <= -1)
            {
                ...
                if (StartRowProperty == PositionProperty.Absolute) sb.Append('$');

                sb.Append("1:");

                if (EndRowProperty == PositionProperty.Absolute) sb.Append('$');
            ...
            if (rows <= -1)
            {
                ...
                if (StartColumnProperty == PositionProperty.Absolute) sb.Append('$');

                sb.Append(RGUtility.GetAlphaChar(col));
                sb.Append(':');

                if (EndColumnProperty == PositionProperty.Absolute) sb.Append('$');

                sb.Append(RGUtility.GetAlphaChar(EndCol));
        ...
        public string ToRelativeAddress()
        {
            if (rows <= -1 && cols <= -1)
                return "1:1048576";
            if (cols <= -1)
                return string.Format("{0}:{1}", row + 1, EndRow + 1);
            if (rows <= -1)
                return string.Format("{0}:{1}", RGUtility.GetAlphaChar(col), RGUtility.GetAlphaChar(EndCol));
            return string.Format("{0}{1}:{2}{3}", RGUtility.GetAlphaChar(col), row + 1,
                RGUtility.GetAlphaChar(EndCol), EndRow + 1);
        }
        ...
        public static RangePosition Union(RangePosition range1, RangePosition range2)
        {
            ...
            var row = Math.Min(range1.row, range2.row);
            var col = Math.Min(range1.col, range2.col);
            var endrow = Math.Max(range1.EndRow, range2.EndRow);
            var endcol = Math.Max(range1.EndCol, range2.EndCol);
