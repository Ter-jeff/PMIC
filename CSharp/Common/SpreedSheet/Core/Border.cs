...
namespace unvell.ReoGrid
{
    partial class Worksheet
    {
        ...
        /// </summary>
        /// <param name="scanDirections">
        ///     Specifies that borders iterated on which direction, horizontal borders,
        ///     vertical borders or both horizontal and vertical borders.
        /// </param>
        ...
        public void IterateBorders(RowOrColumn scanDirections, RangePosition range,
            Func<int, int, int, RangeBorderStyle, bool> iterator)
        {
            range = FixRange(range);

            if ((scanDirections & RowOrColumn.Row) == RowOrColumn.Row)
                for (var r = range.Row; r <= range.EndRow; r++)
                for (var c = range.Col; c <= range.EndCol;)
                {
                    var border = hBorders[r, c];

                    if (border != null)
                        if (border.Span > 0)
                        {
                            if (!iterator(r, c, border.Span, border.Style)) goto outLoopRow;
            ...
            if ((scanDirections & RowOrColumn.Column) == RowOrColumn.Column)
                ...
                for (var r = range.Row; r <= range.EndRow;)
                {
                    var border = vBorders[r, c];

                    if (border != null)
                        if (border.Span > 0)
                        {
                            if (!iterator(r, c, border.Span, border.Style)) goto outLoopColumn;
        ...
        {
            SetRangeBorders(new RangePosition(row, col, rows, cols), pos, style);
        }
        ...
        {
            SetRangeBorders(FixRange(range), pos, style, true);
        }

        internal void SetRangeBorders(RangePosition range, BorderPositions pos, RangeBorderStyle style,
            bool invalidateWorksheet)
        {
            range = FixRange(range);

            var r1 = range.Row;
            var c1 = range.Col;
            var r2 = range.EndRow + 1;
            var c2 = range.EndCol + 1;
            ...
            if (invalidateWorksheet) RequestInvalidate();
        ...
        /// <param name="pos">
        ///     Specifies where to get the borders around the range or cell. For example,
        ///     pass <code>BorderPositions.Outside</code> to get outside borders around specified address;
        ///     pass <code>BorderPositions.All</code> to get all borders from the range.
        /// </param>
        ...
        {
            return GetRangeBorders(new RangePosition(row, col, rows, cols), pos, onlyCellsOwn);
        }
        ...
        /// <param name="onlyCellsOwn">True to get the borders only owned by the range or cell.</param>
        /// <returns>Borders information object.</returns>
        public RangeBorderInfoSet GetRangeBorders(string addressOrName, BorderPositions pos = BorderPositions.All,
            bool onlyCellsOwn = true)
        {
            NamedRange namedRange;
            if (RangePosition.IsValidAddress(addressOrName))
                return GetRangeBorders(new RangePosition(addressOrName), pos, onlyCellsOwn);
            if (NamedRange.IsValidName(addressOrName)
                && registeredNamedRanges.TryGetValue(addressOrName, out namedRange))
        ...
        public RangeBorderInfoSet GetRangeBorders(RangePosition range, BorderPositions pos = BorderPositions.All,
            bool onlyCellsOwn = true)
        {
            var fixedRange = FixRange(range);

            var borderInfo = new RangeBorderInfoSet();
            ...
            {
                borderInfo.Top = GetGridBorder(fixedRange.Row, fixedRange.Col, BorderPositions.Top, onlyCellsOwn);
                for (var c = fixedRange.Col + 1; c <= fixedRange.EndCol; c++)
            ...
            {
                borderInfo.Bottom =
                    GetGridBorder(fixedRange.EndRow, fixedRange.Col, BorderPositions.Bottom, onlyCellsOwn);
            ...
            {
                borderInfo.Left = GetGridBorder(fixedRange.Row, fixedRange.Col, BorderPositions.Left, onlyCellsOwn);
                if (borderInfo.Left != null)
                {
                    for (var r = fixedRange.Row + 1; r < fixedRange.EndRow; r++)
            ...
            {
                borderInfo.Right =
                    GetGridBorder(fixedRange.Row, fixedRange.EndCol, BorderPositions.Right, onlyCellsOwn);
            ...
            {
                // inside horizontal
                var hbEndRow = Math.Min(fixedRange.EndRow, hBorders.MaxRow);
                var hbEndCol = Math.Min(fixedRange.EndCol, hBorders.MaxCol);

                for (var r = fixedRange.Row + 1; r <= hbEndRow; r++)
                for (var c = fixedRange.Col; c <= hbEndCol;)
                {
                    var hBorder = hBorders[r, c];
                    if (hBorder != null)
                    {
                        if (hBorder.Span == 0)
                        {
            ...
            {
                // inside vertical
                var vbEndRow = Math.Min(fixedRange.EndRow, vBorders.MaxRow);
                var vbEndCol = Math.Min(fixedRange.EndCol, vBorders.MaxCol);

                for (var c = fixedRange.Col + 1; c <= vbEndCol; c++)
                for (var r = fixedRange.Row; r <= vbEndRow;)
                {
                    var vBorder = vBorders[r, c];
                    if (vBorder != null)
                    {
                        if (vBorder.Span == 0)
                        {
        ...
        {
            var border = hBorders[row, col];
            if (border == null) border = hBorders[row, col] = new ReoGridHBorder();
            return border;
        }

        internal ReoGridVBorder GetVBorder(int row, int col)
        {
            var border = vBorders[row, col];
            if (border == null) border = vBorders[row, col] = new ReoGridVBorder();
            return border;
        }
        ...
        private void CutBeforeHBorder(int row, int col)
        {
            ...
            var prevBorder = hBorders[row, col - 1];
            if (prevBorder != null && prevBorder.Span > 1)
            {
                var c = col - 1;
                var offset = prevBorder.Span;
                while (c >= 0)
                {
        ...
        private int FindSameHBorderLeft(int row, int col, RangeBorderStyle borderStyle)
        {
            ...
            for (var c = col - 1; c >= 0; c--)
            {
                ...
                if (hb == null)
                    return c + 1;
                if (hb.Style == null && borderStyle != null)
                    return c + 1;
                //else if (hBorders[row, c].Border != null && borderStyle == null)
                //  return c + 1;
                if (hb.Style != null && !hb.Style.Equals(borderStyle))
                    return c + 1;
            }
        ...
        {
            if (col > cols.Count - 1) return col;

            for (var c = col + 1; c < cols.Count; c++)
            {
                var hb = hBorders[row, c];

                if (hb == null)
                    return c - 1;
                if (hb == null && borderStyle != null)
                    return c - 1;
                if (!hb.Style.Equals(borderStyle))
                    return c - 1;
            }
        ...
        {
            var tc = col + cols;
            for (var c = col; c < tc; c++) SetHBorder(row, c, tc - c, borderStyle, pos);
        }
        ...
        private void SetHBorders(int row, int col, int cols, RangeBorderStyle borderStyle, HBorderOwnerPosition pos)
        {
            ...
            for (var k = sc; k < c2; k++)
                if (row > 0)
                {
                    var cell1 = cells[row - 1, k];
                    var cell2 = cells[row, k];
            ...
            if (nextStartCol != -1) SetHBorders(row, nextStartCol, c2 - nextStartCol, borderStyle, pos);
        }
        ...
        private void SetHBorder(int row, int col, int colspan, RangeBorderStyle borderStyle, HBorderOwnerPosition pos)
        {
            ...
            {
                // set border style
                var hBorder = GetHBorder(row, col);
        ...
        private int FindSameVBorderTop(int row, int col, RangeBorderStyle borderStyle)
        {
            ...
            for (var r = row - 1; r >= 0; r--)
            {
                ...
                if (vb == null)
                    return r + 1;
                if (vb.Style == null && borderStyle != null)
                    return r + 1;
                //else if (vBorders[r, col].Border != null && borderStyle == null)
                //  return r + 1;
                if (vb.Style != null && !vb.Style.Equals(borderStyle))
                    return r + 1;
            }
        ...
        {
            if (row > rows.Count - 1) return row;

            for (var r = row + 1; r < rows.Count; r++)
            {
                var vb = vBorders[r, col];
                ...
                if (vb.Style == null && borderStyle != null)
                    return r - 1;
                if (!vb.Style.Equals(borderStyle))
                    return r - 1;
            }
        ...
        {
            var tr = row + rows;
            for (var r = row; r < tr; r++) SetVBorder(r, col, tr - r, borderStyle, pos);
        }
        ...
        private void CutBeforeVBorder(int row, int col)
        {
            ...
            var prevBorder = vBorders[row - 1, col];
            if (prevBorder != null && prevBorder.Span > 1)
            {
                var r = row - 1;
                var offset = prevBorder.Span;
        ...
        private void SetVBorders(int row, int col, int rows, RangeBorderStyle borderStyle, VBorderOwnerPosition pos)
        {
            ...
            for (var k = sr; k < r2; k++)
                if (col > 0)
                {
                    var cell1 = cells[k, col - 1];
                    var cell2 = cells[k, col];
            ...
            if (nextStartRow != -1) SetVBorders(nextStartRow, col, r2 - nextStartRow, borderStyle, pos);
        }
        ...
        private void SetVBorder(int row, int col, int rowspan, RangeBorderStyle borderStyle, VBorderOwnerPosition pos)
        {
            ...
            {
                // set border style
                var vBorder = GetVBorder(row, col);
    ...
    public class RangeBorderInfoSet
    {
        ...
        /// </summary>
        public RangeBorderStyle Backslash
        {
            get { return backslash; }
            set { backslash = value; }
        }
    ...
    public class CellBorderProperty
    {
        ...
        public RangeBorderStyle Left
        {
            get
            {
                ...
                return cell.Worksheet.GetGridBorder(cell.Row, cell.Column, BorderPositions.Left, true);
            }
            set
            {
                CheckForOwnerAssociated();

                cell.Worksheet.SetRangeBorders(cell.PositionAsRange, BorderPositions.Left, value);
            }
        }
        ...
        public RangeBorderStyle Top
        {
            get
            {
                ...
                return cell.Worksheet.GetGridBorder(cell.Row, cell.Column, BorderPositions.Top, true);
            }
            set
            {
                CheckForOwnerAssociated();

                cell.Worksheet.SetRangeBorders(cell.PositionAsRange, BorderPositions.Top, value);
            }
        }
        ...
        public RangeBorderStyle Right
        {
            get
            {
                ...
                return cell.Worksheet.GetGridBorder(cell.Row, cell.Column, BorderPositions.Right, true);
            }
            set
            {
                CheckForOwnerAssociated();

                cell.Worksheet.SetRangeBorders(cell.PositionAsRange, BorderPositions.Right, value);
            }
        }
        ...
        public RangeBorderStyle Bottom
        {
            get
            {
                ...
                return cell.Worksheet.GetGridBorder(cell.Row, cell.Column, BorderPositions.Bottom, true);
            }
            set
            {
                CheckForOwnerAssociated();

                cell.Worksheet.SetRangeBorders(cell.PositionAsRange, BorderPositions.Bottom, value);
            }
        }
        ...
        public RangeBorderStyle Outside
        {
            get
            {
                ...
                return cell.Worksheet.GetGridBorder(cell.Row, cell.Column, BorderPositions.Outside, true);
            }
            set
            {
                CheckForOwnerAssociated();

                cell.Worksheet.SetRangeBorders(cell.PositionAsRange, BorderPositions.Outside, value);
            }
        }
        ...
        public RangeBorderStyle All
        {
            get
            {
                ...
                return cell.Worksheet.GetGridBorder(cell.Row, cell.Column, BorderPositions.All, true);
            }
            set
            {
                CheckForOwnerAssociated();

                cell.Worksheet.SetRangeBorders(cell.Row, cell.Column, 1, 1, BorderPositions.All, value);
            }
        }
        ...
        private void CheckForOwnerAssociated()
        {
            if (cell == null)
                throw new ReferenceObjectNotAssociatedException(
                    "Border property is not associated to any cells. Border might have been deleted from the cell. Try get this property from cell again.");
        }
    }
    ...
    public class RangeBorderProperty
    {
        ...
        public RangeBorderStyle Left
        {
            get
            {
                ...
                return range.Worksheet.GetRangeBorders(Position, BorderPositions.Left).Left;
            }
            set
            {
                CheckForOwnerAssociated();

                range.Worksheet.SetRangeBorders(Position, BorderPositions.Left, value);
            }
        }
        ...
        public RangeBorderStyle Top
        {
            get
            {
                ...
                return range.Worksheet.GetRangeBorders(Position, BorderPositions.Top).Top;
            }
            set
            {
                CheckForOwnerAssociated();

                range.Worksheet.SetRangeBorders(Position, BorderPositions.Top, value);
            }
        }
        ...
        public RangeBorderStyle Right
        {
            get
            {
                ...
                return range.Worksheet.GetRangeBorders(Position, BorderPositions.Right).Right;
            }
            set
            {
                CheckForOwnerAssociated();

                range.Worksheet.SetRangeBorders(Position, BorderPositions.Right, value);
            }
        }
        ...
        public RangeBorderStyle Bottom
        {
            get
            {
                ...
                return range.Worksheet.GetRangeBorders(Position, BorderPositions.Bottom).Bottom;
            }
            set
            {
                CheckForOwnerAssociated();

                range.Worksheet.SetRangeBorders(Position, BorderPositions.Bottom, value);
            }
        }
        ...
        public RangeBorderStyle Inside
        {
            get
            {
                ...
                return range.Worksheet.GetRangeBorders(Position, BorderPositions.InsideAll)
            ...
            set
            {
                ...
                range.Worksheet.SetRangeBorders(Position, BorderPositions.InsideAll, value);
            }
        }
        ...
        public RangeBorderStyle InsideHorizontal
        {
            get
            {
                ...
                return range.Worksheet.GetRangeBorders(Position, BorderPositions.InsideHorizontal).InsideHorizontal;
            }
            set
            {
                CheckForOwnerAssociated();

                range.Worksheet.SetRangeBorders(Position, BorderPositions.InsideHorizontal, value);
            }
        }
        ...
        public RangeBorderStyle InsideVertical
        {
            get
            {
                ...
                return range.Worksheet.GetRangeBorders(Position, BorderPositions.InsideVertical).InsideVertical;
            }
            set
            {
                CheckForOwnerAssociated();

                range.Worksheet.SetRangeBorders(Position, BorderPositions.InsideVertical, value);
            }
        }
        ...
        public RangeBorderStyle Outside
        {
            get
            {
                ...
                return range.Worksheet.GetRangeBorders(Position, BorderPositions.Outside)
            ...
            set
            {
                ...
                range.Worksheet.SetRangeBorders(Position, BorderPositions.Outside, value);
            }
        }
        ...
        public RangeBorderStyle All
        {
            get
            {
                ...
                return range.Worksheet.GetRangeBorders(Position).Left; // TODO: no outline available here
            }
            set
            {
                CheckForOwnerAssociated();

                range.Worksheet.SetRangeBorders(Position, BorderPositions.All, value);
            }
        }
        ...
        private void CheckForOwnerAssociated()
        {
            if (range == null)
                throw new ReferenceObjectNotAssociatedException(
                    "Border property is not associated to any ranges. Border might have been deleted from range. Try get this property from range again.");
        }
    }
}