...
#if WINFORM || WPF
using ControlType = unvell.ReoGrid.SheetControl;
#elif ANDROID
...
namespace SpreedSheet.Core.Workbook
{
...
#endif // WINFORM || WPF
    {
        ...
        void Load(Stream stream, FileFormat fileFormat, string sheetName);
        ...
        /// <param name="encoding">Encoding used to read plain-text format workbook data from stream. (Optional)</param>
        /// <param name="sheetName"></param>
        void Load(Stream stream, FileFormat fileFormat, Encoding encoding, string sheetName);
