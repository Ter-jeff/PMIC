...
namespace unvell.ReoGrid
{
        ...
        public bool Equals(PartialGrid anotherPartialGrid, PartialGridCopyFlag flag)
        {
            ...
            for (var c = 0; c < cols; c++)
            {
                ...
                if ((flag & PartialGridCopyFlag.CellStyle) == PartialGridCopyFlag.CellStyle
                    && Equals(a.InnerStyle, b.InnerStyle))
                    return false;
    ...
    partial class Worksheet
    {
        ...
        {
            if (RangePosition.IsValidAddress(addressOrName)) return GetPartialGrid(new RangePosition(addressOrName));

            NamedRange namedRange;
            if (TryGetNamedRange(addressOrName, out namedRange))
                return GetPartialGrid(namedRange.Position);
            ...
            bool checkIntersectedRange = false)
        {
            ...
            if (checkIntersectedRange)
            {
                ...
                if (intersectedRange != RangePosition.Empty) throw new RangeIntersectionException(intersectedRange);
            }

            var rows = range.Rows;
            var cols = range.Cols;

            var data = new PartialGrid
            {
                Columns = cols,
                Rows = rows
            };
                ...
                || (flag & PartialGridCopyFlag.CellStyle) == PartialGridCopyFlag.CellStyle)
            {
                ...
                for (var c = range.Col; c <= range.EndCol; c++)
                {
                    ...
                    var toRow = r - range.Row;
                    var toCol = c - range.Col;
                    ...
                    else
                    {
                        ...
                        style = StyleUtility.DistinctStyle(style, DefaultStyle);
                    ...
                    if (toCell != null) data.Cells[toRow, toCol] = toCell;
            ...
            if ((flag & PartialGridCopyFlag.HBorder) == PartialGridCopyFlag.HBorder)
            {
                ...
                hBorders.Iterate(range.Row, range.Col, rows + 1, cols, true, (r, c, hBorder) =>
                {
                    ...
                    {
                        var toCol = c - range.Col;
                        var thBorder = ReoGridHBorder.Clone(hBorder);
            ...
            if ((flag & PartialGridCopyFlag.VBorder) == PartialGridCopyFlag.VBorder)
            {
                ...
                vBorders.Iterate(range.Row, range.Col, rows, cols + 1, true, (r, c, vBorder) =>
                {
                    ...
                    {
                        var toRow = r - range.Row;
                        var tvBorder = ReoGridVBorder.Clone(vBorder);
        ...
        public RangePosition SetPartialGrid(string addressOrName, PartialGrid data)
        {
            if (RangePosition.IsValidAddress(addressOrName))
                return SetPartialGrid(new RangePosition(addressOrName), data);

            NamedRange namedRange;
            if (TryGetNamedRange(addressOrName, out namedRange))
                return SetPartialGrid(namedRange.Position, data);
            throw new InvalidAddressException(addressOrName);
        }
            ...
            PartialGridCopyFlag flag, ExPartialGridCopyFlag exFlag)
        {
            ...
            if ((flag & PartialGridCopyFlag.CellData) == PartialGridCopyFlag.CellData
                ...
                for (var c = 0; c < cols; c++)
                {
                    var fromCell = data.Cells == null ? null : data.Cells[r, c];

                    var tr = toRange.Row + r;
                    var tc = toRange.Col + c;
                    ...
                    if (fromCell != null)
                    {
                            ...
                            fromCell.MergeEndPos.Col <= toRange.EndCol)
                        {
                            ...
                            var fromMergedStart = CreateAndGetCell(fromCell.MergeStartPos);
                            ...
                            for (var ic = fromMergedStart.InternalCol;
                                 ic < fromMergedStart.InternalCol + fromMergedStart.Colspan;
                                 ic++)
                            {
                        ...
                        )
                        {
                            ...
                            var rightCol = Math.Min(fromCell.MergeEndPos.Col, this.cols.Count - 1);

                            var tocell = CreateAndGetCell(tr, tc);
                            tocell.MergeStartPos = new CellPosition(fromCell.MergeStartPos.Row,
                                fromCell.MergeStartPos.Col + tc - fromCell.InternalCol);
                            tocell.MergeEndPos = new CellPosition(fromCell.MergeEndPos.Row, rightCol);

                            for (var ic = toRange.EndCol + 1; ic <= rightCol; ic++)
                            {
                            ...
                            && fromCell.MergeEndPos.Row <= toRange.EndRow)
                        {
                            ...
                            for (var ir = fromCell.MergeStartPos.Row; ir < tr; ir++)
                            {
                            ...
                            if (tocell.IsEndMergedCell)
                                mergedStartCell.Bounds = GetRangeBounds(mergedStartCell.InternalPos,
                                    mergedStartCell.MergeEndPos);
                            ...
                            && fromCell.MergeStartPos.Row <= toRange.EndRow)
                        {
                            // target partial range will override exsited range
                            // need to update existed range at right side
                            var bottomRow = Math.Min(fromCell.MergeEndPos.Row, this.rows.Count - 1);

                            for (var ir = toRange.EndRow + 1; ir <= bottomRow; ir++)
                            {
                                var existedEndCell = CreateAndGetCell(ir, tc);
                                existedEndCell.MergeStartPos = new CellPosition(fromCell.MergeStartPos.Row,
                                    existedEndCell.MergeStartPos.Col);
                                existedEndCell.Rowspan = 0;
                                existedEndCell.Colspan = 0;
                            }
                    ...
                    if (!processed)
                    {
                        ...
                        if (toCell.Rowspan == 0 && toCell.Colspan == 0) continue;
								...
								if ((flag & PartialGridCopyFlag.CellFormula) == PartialGridCopyFlag.CellFormula)
								{
									if (fromCell.HasFormula)
									{
										if (fromCell.formulaTree == null)
										{
											try
											{
												fromCell.formulaTree =
 Formula.Parser.Parse(this.workbook, fromCell, fromCell.InnerFormula);
											}
											catch
											{
                            ...
                            if (toCell.Rowspan == 1 && toCell.Colspan == 1)
                            {
                                ...
                                {
                                    toCell.MergeStartPos = fromCell.MergeStartPos.Offset(tr - fromCell.InternalRow,
                                        tc - fromCell.InternalCol);

#if DEBUG
                                    Debug.Assert(toCell.MergeStartPos.Row >= 0 &&
                                                 toCell.MergeStartPos.Row < this.rows.Count);
                                    Debug.Assert(toCell.MergeStartPos.Col >= 0 &&
                                                 toCell.MergeStartPos.Col < this.cols.Count);
#endif
                                }

                                if (!fromCell.MergeEndPos.IsEmpty)
                                {
                                    toCell.MergeEndPos = fromCell.MergeEndPos.Offset(tr - fromCell.InternalRow,
                                        tc - fromCell.InternalCol);

#if DEBUG
                                    Debug.Assert(
                                        toCell.MergeEndPos.Row >= 0 && toCell.MergeEndPos.Row < this.rows.Count);
                                    Debug.Assert(
                                        toCell.MergeEndPos.Col >= 0 && toCell.MergeEndPos.Col < this.cols.Count);
#endif
                                }
                            }
                            else
                                ...
                                && fromCell.InnerStyle != null)
                            {
                                ...
                                if (fromCell.InnerStyle.HAlign == GridHorAlign.General)
                            ...
                            if (toCell.IsEndMergedCell)
                            {
                                var cell = GetCell(toCell.MergeStartPos);
            ...
            if ((flag & PartialGridCopyFlag.HBorder) == PartialGridCopyFlag.HBorder)
            {
                if (data.HBorders == null)
                {
                    ...
                    // set borders to null
                    hBorders.Iterate(toRange.Row, toRange.Col, rows, cols, true, (r, c, fromHBorder) =>
                        {
                            hBorders[r, c] = null;
                            return 1;
                        }
                ...
                else
                {
                    ...
                    for (var c = 0; c < cols; c++)
                    {
                        ...
                        {
                            var style = fromHBorder.Style;

                            var hcols = fromHBorder.Span;
                            if (hcols > cols - c) hcols = cols - c;

                            GetHBorder(tr, tc).Span = hcols;
            ...
            if ((flag & PartialGridCopyFlag.VBorder) == PartialGridCopyFlag.VBorder)
            {
                if (data.VBorders == null)
                {
                    ...
                    // set border to null
                    vBorders.Iterate(toRange.Row, toRange.Col, rows, cols, true, (r, c, fromVBorder) =>
                        {
                            vBorders[r, c] = null;
                            return 1;
                        }
                ...
                else
                {
                    ...
                    for (var c = 0; c < cols + 1; c++)
                    {
                        ...
                        {
                            var style = fromVBorder.Style;

                            var vrows = fromVBorder.Span;
                            ...
                            if (data.VBorders[r, c].Style != null)
                                ...
                                vBorders[tr, tc].Style = RangeBorderStyle.Empty;
                        }
                    }
                }
            }
        ...
        public RangePosition SetPartialGridRepeatly(string addressOrName, PartialGrid grid)
        {
            ...
            if (RangePosition.IsValidAddress(addressOrName))
                return SetPartialGridRepeatly(new RangePosition(addressOrName), grid);
            if (TryGetNamedRange(addressOrName, out namedRange))
                return SetPartialGridRepeatly(namedRange.Position, grid);
            throw new InvalidAddressException(addressOrName);
        }
        ...
        public RangePosition SetPartialGridRepeatly(RangePosition range, PartialGrid grid)
        {
            ...
            for (var r = range.Row; r <= range.EndRow; r += grid.Rows)
            for (var c = range.Col; c <= range.EndCol; c += grid.Columns)
