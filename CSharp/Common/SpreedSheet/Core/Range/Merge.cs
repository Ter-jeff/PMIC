...
namespace unvell.ReoGrid
{
    partial class Worksheet
    {
        ...
        /// </summary>
        /// <param name="addressOrName">
        ///     address or name to locate a range, if a name of range is specified,
        ///     the name must be defined by DefineNamedRange first.
        /// </param>
        ...
        internal void MergeRange(RangePosition range, bool checkIntersection = true, bool updateUIAndEvent = true)
        {
            ...
            if (fixedRange.Cols <= 1 && fixedRange.Rows <= 1) return;

            if (checkIntersection)
            {
                var intersectedRange = CheckIntersectedMergingRange(fixedRange);
                if (!intersectedRange.IsEmpty) throw new RangeIntersectionException(intersectedRange);
            }

            var row = fixedRange.Row;
            var col = fixedRange.Col;
            var torow = fixedRange.EndRow;
            var tocol = fixedRange.EndCol;

            // find start and end cell
            var startCell = cells[row, col];
            var endCell = cells[torow, tocol];

            if (startCell == null) startCell = CreateCell(row, col);
            if (endCell == null) endCell = CreateCell(torow, tocol);

            for (var r = row; r <= torow; r++)
            for (var c = col; c <= tocol; c++)
            {
            ...
            // fix selection
            if (selectionRange.IntersectWith(fixedRange)) selectionRange = CheckMergedRange(fixedRange);

            // fix focus pos
            if (fixedRange.Contains(focusPos)) focusPos = fixedRange.StartPos;

            if (updateUIAndEvent)
            {
                RequestInvalidate();

                if (RangeMerged != null) RangeMerged(this, new RangeEventArgs(fixedRange));
            }
        }
        ...
        public void UnmergeRange(RangePosition range)
        {
            ...
            var row = range.Row;
            var col = range.Col;
            var torow = range.Row + range.Rows - 1;
            var tocol = range.Col + range.Cols - 1;
        ...
        private void UnmergeCell(Cell source)
        {
            ...
            var r2 = source.InternalRow + source.Rowspan;
            var c2 = source.InternalCol + source.Colspan;

            var range = new RangePosition(source.InternalRow, source.InternalCol, source.Rowspan, source.Colspan);

            for (var r = source.InternalRow; r < r2; r++)
            for (var c = source.InternalCol; c < c2; c++)
            {
                var cell = CreateAndGetCell(r, c);
            ...
            if (RangeUnmerged != null) RangeUnmerged(this, new RangeEventArgs(range));
        }
        ...
        {
            var intersectedRange = RangePosition.Empty;

            cells.Iterate(range.Row, range.Col, range.Rows, range.Cols, true, (r, c, cell) =>
            {
                if (!cell.MergeStartPos.IsEmpty)
                {
                    var checkStartCell = GetCell(cell.MergeStartPos);
                    for (var rr = checkStartCell.InternalRow; rr <= checkStartCell.MergeEndPos.Row; rr++)
                    {
                        for (var cc = checkStartCell.InternalCol; cc <= checkStartCell.MergeEndPos.Col; cc++)
                        {
                            var targetCell = cells[rr, cc];
                            if (targetCell != null && !range.Contains(targetCell.InternalPos))
                            {
                                intersectedRange = new RangePosition(checkStartCell.InternalPos,
                                    checkStartCell.MergeEndPos);
                                break;
                            }
                        }
        ...
        /// <summary>
        ///     Check whether a  range intersects with others merged range, return the most outside range which contains all these
        ///     intersected range.
        /// </summary>
        /// <param name="range">Range to be checked.</param>
        ...
        public RangePosition CheckMergedRange(RangePosition range)
        {
            var minr = range.Row;
            var minc = range.Col;
            var maxr = range.EndRow;
            var maxc = range.EndCol;
        ...
        {
            var cell = cells[range.Row, range.Col];
