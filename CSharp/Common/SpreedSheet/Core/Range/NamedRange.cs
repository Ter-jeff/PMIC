...
namespace unvell.ReoGrid
{
    ...
    {
        private NamedRangeCollection namedRangeCollection;
        ...
        public NamedRangeCollection NamedRanges
        {
            get
            {
                if (namedRangeCollection == null) namedRangeCollection = new NamedRangeCollection(this);
                return namedRangeCollection;
            }
        }
        ...
        public void AddNamedRange(NamedRange namedRange)
        {
            ...
            if (namedRange.Worksheet != null && namedRange.Worksheet != this)
                throw new ArgumentException(
                    "Specified range is belong to another worksheet, remove from other worksheet firstly, or create a new range at same position.");

            if (namedRange.Scope == NamedRangeScope.Workbook
                && workbook.worksheets.Any(s => s.registeredNamedRanges.Any(p =>
            ...
            registeredNamedRanges[namedRange.Name] = namedRange;
            namedRange.Worksheet = this;

            if (NamedRangeDefined != null) NamedRangeDefined(this, new NamedRangeAddedEventArgs(namedRange));
        }
        ...
        {
            return DefineNamedRange(name, address, NamedRangeScope.Workbook);
        }
        ...
        {
            if (!RangePosition.IsValidAddress(address)) throw new InvalidAddressException(address);
        ...
        {
            return DefineNamedRange(name, new RangePosition(row, col, rows, cols));
        }
        ...
        /// <param name="scope">The valid scope of this named range</param>
        /// <returns>Instance of named range added into spreadsheet</returns>
        public NamedRange DefineNamedRange(string name, RangePosition range,
        ...
        {
            return registeredNamedRanges.Keys;
        }
        ...
        {
            var rr = registeredNamedRanges.FirstOrDefault(nr => nr.Value.StartPos == range.StartPos
        ...
        public bool UndefineNamedRange(string name)
        {
            ...
            if (registeredNamedRanges.TryGetValue(name, out namedRange))
            {
                ...
                if (NamedRangeUndefined != null)
                    NamedRangeUndefined(this, new NamedRangeUndefinedEventArgs(namedRange, namedRange.Name));
        ...
        {
            NamedRange namedRange;
            if (registeredNamedRanges.TryGetValue(oldName, out namedRange))
            {
                namedRange.internalName = newName;
                registeredNamedRanges.Remove(oldName);
                registeredNamedRanges[newName] = namedRange;
    ...
    public class NamedRange : ReferenceRange
    {
        ...
        {
            if (RangePosition.IsValidAddress(name)) throw new InvalidNamedRangeException(name);
        ...
        public string Name
        {
            ...
            set
            {
                if (internalName != value)
                {
                    ...
                    if (Worksheet != null) oldRange = Worksheet.GetNamedRange(internalName);

                    if (oldRange != null)
                        Worksheet.RenameNamedRange(internalName, value);
                    else
                        internalName = value;
        ...
        {
            return string.Format("{0}!{1}", Worksheet.Name, ToAddress());
        }
    }
    ...
    public class NamedRangeCollection : ICollection<NamedRange>
    {
        ...
        public NamedRange this[string name]
        {
            ...
            {
                NamedRange range;
                return sheet.TryGetNamedRange(name, out range) ? range : null;
            }
            set
            {
                if (value.Name != name)
                    throw new ArgumentException(
                        "Specified name is different from the name that is set in the range instance.", "name");

                sheet.AddNamedRange(value);
            }
        }
        ...
        public void Clear()
        {
            sheet.registeredNamedRanges.Clear();
        }
        ...
        {
            return sheet.registeredNamedRanges.Values.Any(nr => nr.Name == range.Name && nr.Position == range.Position);
        }
        ...
        {
            sheet.registeredNamedRanges.Values.CopyTo(array, arrayIndex);
        }
        ...
        public int Count
        {
            get { return sheet.registeredNamedRanges.Values.Count; }
        }
        ...
        public bool Remove(NamedRange range)
        {
            ...
            return sheet.UndefineNamedRange(range.Name);
        }
        ...
        {
            return sheet.registeredNamedRanges.Values.GetEnumerator();
        }
