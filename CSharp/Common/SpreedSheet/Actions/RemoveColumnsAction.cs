...
namespace unvell.ReoGrid.Actions
{
    ...
    public class RemoveColumnsAction : WorksheetReusableAction
    {
        ...
        public override void Do()
        {
            if (Range.Cols == -1)
            {
                Logger.Log("remove columns",
                    "attempt to remove all columns but worksheet must be have at least one column, operation aborted.");
                return;
            }

            backupWidths = new int[Range.Cols];

            for (var i = Range.Col; i <= Range.EndCol; i++)
        ...
        public override void Undo()
        {
            ...
            {
                Logger.Log("remove columns",
                    "attempt to undo removing all columns but grid must have one column, operation aborted.");
                return;
            }

            Worksheet.InsertColumns(Range.Col, Range.Cols);
            Worksheet.SetColumnsWidth(Range.Col, Range.Cols, col => backupWidths[col - Range.Col]);
            ...
            //	}
            //} 
            if (deletedNamedRanges != null)
                foreach (var namedRange in deletedNamedRanges)
            ...
            //	}
            //} 
            if (deletedHighlightRanges != null)
                foreach (var range in deletedHighlightRanges)
