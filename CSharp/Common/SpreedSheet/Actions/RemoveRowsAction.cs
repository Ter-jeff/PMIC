...
namespace unvell.ReoGrid.Actions
{
    ...
    public class RemoveRowsAction : WorksheetReusableAction
    {
        ...
        public override void Do()
        {
            if (Range.Rows == -1)
            {
                Logger.Log("remove rows",
                    "attempt to remove all columns but grid must have one column, operation aborted.");
                return;
            }

            backupHeights = new int[Range.Rows];

            for (var i = Range.Row; i <= Range.EndRow; i++)
        ...
        public override void Undo()
        {
            ...
            {
                Logger.Log("remove rows",
                    "attempt to undo removing all rows from the worksheet must it must be have at least one row, operation aborted.");
                return;
            }
