...
namespace unvell.Common
{
    internal static class GraphicsToolkit
    {
        ...
        public static double DistancePointToLine(double x1, double y1, double x2, double y2, Point target)
        {
            var a = y2 - y1;
            var b = x1 - x2;
            var c = x2 * y1 - x1 * y2;
        ...
        public static double DistancePointToPolygonBound(Point[] points, Point p, double min = 9999999,
            bool firstReturn = false)
        {
            var mindis = min + 1;

            var linePoint = points[points.Length - 1];

            foreach (var linePoint2 in points)
            {
                var dist = DistancePointToLine(linePoint, linePoint2, p);
        ...
        public static double DistancePointToRectBounds(Rectangle rect, Point p, double min = 9999999,
            bool firstReturn = false)
        {
            return DistancePointToPolygonBound(new[]
            {
        ...
        public static bool PointOnRectangleBounds(Rectangle rect, Point p, double borderWidth = 1, double min = 9999999)
        {
            rect.Inflate(borderWidth, borderWidth);
            return rect.Contains(p) && DistancePointToRectBounds(rect, p, min) <= borderWidth;
        }

        private const double PIAngleDelta = Math.PI / 180.0;

        public static float AngleToArc(double width, double height, double angle)
        {
        ...
        public static Point PointAtArc(Rectangle rect, double angle)
        {
            var radians = AngleToArc(
                rect.Width, rect.Height, angle) * PIAngleDelta;

            var ww = rect.Width / 2;
            var hh = rect.Height / 2;

            var x = Math.Sin(radians) * ww;
            var y = Math.Cos(radians) * hh;
        ...
        public static void FillTriangle(PlatformGraphics g, double size, Point loc,
            TriangleDirection dir = TriangleDirection.Down)
        {
#if WINFORM
			var p = System.Drawing.Pens.Black;
#elif WPF
            var p = new RGPen(RGBrushes.Black, 1);
#elif ANDROID
			var p = new RGPen();
        ...
        public static void FillTriangle(PlatformGraphics g, double size, Point loc, TriangleDirection dir, RGPen p)
        {
            var x = loc.X;
            var y = loc.Y;
