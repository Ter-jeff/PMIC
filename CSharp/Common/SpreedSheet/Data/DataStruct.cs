...
namespace unvell.ReoGrid.Data
{
        ...
        sealed class TriangleTreeArray<T>
    {
        ...
        public T this[int row, int col]
        {
            get
            {
                ...
                var r2 = (row >> 6) % RowSize;
                var c2 = (col >> 4) % ColSize;
                var page2 = page1[r2, c2];
        ...
        sealed class RegularTreeArray<T>
    {
        ...
        public T this[int row, int col]
        {
            get
            {
                ...
                {
                    var r = (row >> (RowBitLen * d)) % RowSize;
                    var c = (col >> (ColBitLen * d)) % ColSize;
        ...
        {
            get { return (long)Math.Pow(RowSize, MaxDepth); }
        }

        public long ColCapacity
        {
            get { return (long)Math.Pow(ColSize, MaxDepth); }
        }
        ...
        public bool TryGet(int row, int col, out T value)
        {
            ...
            var obj = node.data[row % RowSize, col % ColSize];
            if (obj == null)
            {
                value = default(T);
                return false;
        ...
        public void Iterate(int startRow, int startCol, int rows, int cols, bool ignoreNull,
            Func<int, int, T, int> iterator)
        {
            var r2 = startRow + rows;
            var c2 = startCol + cols;

            for (var r = startRow; r < r2; r++)
            for (var c = startCol; c < c2;)
                if (IsPageNull(r, c))
                {
                    c += ColSize - c % ColSize;
                }
        ...
        internal void IterateEx(int startRow, int startCol, int endRow, int endCol, Func<int, int, bool> isTarget,
            Func<int, int, T, int> iterator)
        {
        ...
        private bool IterateTree(Node node, int offrow, int offcol, Func<int, int, T, bool> handler)
        {
            if (node.nodes != null)
                ...
                for (var c = ColSize; c >= 0; c--)
                {
                    var ret = IterateTree(node.nodes[r, c], offrow + r, offcol + c, handler);
                    if (!ret) return false;
                }
            ...
            if (node.data != null)
                ...
                for (var c = ColSize; c >= 0; c--)
                {
                    var rs = handler(offrow + r, offcol + c, node.data[r, c]);
                    if (!rs) return false;
                }
        ...
        sealed class Index4DArray<T>
    {
        ...
        public T this[int row, int col]
        {
            get
            {
                var l1row = row >> RowSizeBits;
                var l1cols = rows[l1row];
                if (l1cols == null) return default(T);

                var l1col = col >> ColSizeBits;
                var l2rows = l1cols[l1col];
                if (l2rows == null) return default(T);

                var l2row = row & RowSizeModBits; // % RowSize;
                var l2cols = l2rows[l2row];
                if (l2cols == null) return default(T);

                var l2col = col & ColSizeModBits; // % ColSize;
                return l2cols[l2col];
            }
            set
            {
            ...
            for (var r = row; r < endrow; r++)
            {
                ...
                if (l1cols == null)
                {
                    if (!ignoreNull)
                    {
                        ...
                        for (var c = col; c < endcol;)
                        {
                            var cspan = iterator(r, c, dt);
                            if (cspan < 1) return;
                            c += cspan;
                        }
                    }
                ...
                for (var c = col; c < endcol;)
                {
                    ...
                    if (l2rows == null)
                    {
                        ...
                        {
                            var cspan = iterator(r, c, default(T));
                            if (cspan < 1) return;
                            c += cspan;
                        }
    ...
    public sealed class JaggedTreeArray<T>
    {
        ...
        public long RowCapacity
        {
            get { return (long)Math.Pow(RowSize, MaxDepth + 1); }
        }

        public long ColCapacity
        {
            get { return (long)Math.Pow(ColSize, MaxDepth + 1); }
        }
    ...
    internal sealed class TreeArray<T>
    {
        ...
        public TreeArray(int nodeSize, int maxDepth)
        {
            ...
            Capacity = (long)Math.Pow(nodeSize, maxDepth);
        }
