...
namespace IgxlData.Others.PatternListCsvFile
{
    public class PatternNameInfo
    {
        ...
        public PatternNameInfo(string patName, string regDateCode = @"\d{10}")
        {
            ...
            if (Regex.IsMatch(patName,
                    @"_MOD.?")) //必須放在Full Name前 因為Alignment比的是Full Name, 加上.?因為有可能_MOD_XXX
            {
            ...
            if (Regex.IsMatch(patName, "_DM_", RegexOptions.IgnoreCase)) OpCode = "DUAL";
            if (Regex.IsMatch(patName, "_SI_", RegexOptions.IgnoreCase)) OpCode = "SINGLE";
