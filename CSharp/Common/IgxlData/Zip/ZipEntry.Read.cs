...
namespace IgxlData.Zip
{
    public partial class ZipEntry
    {
        ...
        private static bool ReadHeader(ZipEntry ze, Encoding defaultEncoding)
        {
            ...
            if (IsNotValidSig(signature))
            {
                ...
                if (IsNotValidZipDirEntrySig(signature) && signature != ZipConstants.EndOfCentralDirectorySignature)
                    throw new BadReadException(string.Format("  Bad signature (0x{0:X8}) at position  0x{1:X8}",
                        signature, ze.ArchiveStream.Position));
                return false;
            }

            var block = new byte[26];
            var n = ze.ArchiveStream.Read(block, 0, block.Length);
            ...
            {
                ze._Encryption_FromZipFile =
                    ze._Encryption =
                        EncryptionAlgorithm.PkzipWeak; // this *may* change after processing the Extra field
            ...
            // the three amigos may contain marker values for ZIP64.  So we must read them.
            {
                ...
                ze._CompressedSize = (uint) (block[i++] + block[i++] * 256 + block[i++] * 256 * 256 +
                                             block[i++] * 256 * 256 * 256);
                ze._UncompressedSize = (uint) (block[i++] + block[i++] * 256 + block[i++] * 256 * 256 +
                                               block[i++] * 256 * 256 * 256);
            ...
            if (!ze._FileNameInArchive.EndsWith("/") && (ze._BitField & 0x0008) == 0x0008)
            {
                ...
                // workitem 8098: ok (restore)
                var posn = ze.ArchiveStream.Position;
                ...
                while (wantMore)
                {
                    ...
                    var d = SharedUtilities.FindSignature(ze.ArchiveStream,
                    ...
                    if (ze._InputUsesZip64)
                    {
                        ...
                        i = 0;
                        ze._Crc32 = block[i++] + block[i++] * 256 + block[i++] * 256 * 256 +
                                    block[i++] * 256 * 256 * 256;
        ...
        internal static ZipEntry ReadEntry(ZipContainer zc, bool first)
        {
            var zf = zc.ZipFile;
            var s = zc.ReadStream;
            var defaultEncoding = zc.AlternateEncoding;
        ...
        internal static int FindExtraFieldSegment(byte[] extra, int offx, ushort targetHeaderId)
        {
            ...
            {
                var headerId = (ushort) (extra[j++] + extra[j++] * 256);
                if (headerId == targetHeaderId) return j - 2;

                // else advance to next segment
                var dataSize = (short) (extra[j++] + extra[j++] * 256);
                j += dataSize;
            }
        ...
        internal int ProcessExtraField(Stream s, short extraFieldLength)
        {
            ...
            {
                var buffer = _Extra = new byte[extraFieldLength];
                additionalBytesRead = s.Read(buffer, 0, buffer.Length);
                var posn = s.Position - additionalBytesRead;
                var j = 0;
                while (j + 3 < buffer.Length)
                {
                    var start = j;
                    var headerId = (ushort) (buffer[j++] + buffer[j++] * 256);
                    var dataSize = (short) (buffer[j++] + buffer[j++] * 256);
        ...
        private int ProcessExtraFieldZip64(byte[] buffer, int j, short dataSize, long posn)
        {
            ...
            var slurp = new Func<long>(() =>
            {
                if (remainingData < 8)
                    throw new BadReadException(string.Format("  Missing data for ZIP64 extra field, position 0x{0:X16}",
                        posn));
        ...
        private int ProcessExtraFieldInfoZipTimes(byte[] buffer, int j, short dataSize, long posn)
        {
            if (dataSize != 12 && dataSize != 8)
                throw new BadReadException(string.Format(
                    "  Unexpected size (0x{0:X4}) for InfoZip v1 extra field at position 0x{1:X16}", dataSize, posn));

            var timet = BitConverter.ToInt32(buffer, j);
            _Mtime = _unixEpoch.AddSeconds(timet);
            j += 4;

            timet = BitConverter.ToInt32(buffer, j);
            _Atime = _unixEpoch.AddSeconds(timet);
            j += 4;
        ...
        private int ProcessExtraFieldUnixTimes(byte[] buffer, int j, short dataSize, long posn)
        {
            ...
            if (dataSize != 13 && dataSize != 9 && dataSize != 5)
                throw new BadReadException(string.Format(
                    "  Unexpected size (0x{0:X4}) for Extended Timestamp extra field at position 0x{1:X16}", dataSize,
                    posn));
            ...
            if (dataSize == 13 || _readExtraDepth > 0)
            {
                ...
                _Atime = (flag & 0x0002) != 0 && remainingData >= 4
                    ? slurp()
                    : DateTime.UtcNow;

                _Ctime = (flag & 0x0004) != 0 && remainingData >= 4
                    ? slurp()
                    : DateTime.UtcNow;
        ...
        private int ProcessExtraFieldWindowsTimes(byte[] buffer, int j, short dataSize, long posn)
        {
            ...
            if (dataSize != 32)
                throw new BadReadException(string.Format(
                    "  Unexpected size (0x{0:X4}) for NTFS times extra field at position 0x{1:X16}", dataSize, posn));

            j += 4; // reserved
            var timetag = (short) (buffer[j] + buffer[j + 1] * 256);
            var addlsize = (short) (buffer[j + 2] + buffer[j + 3] * 256);
            j += 4; // tag and size

            if (timetag == 0x0001 && addlsize == 24)
            {
                var z = BitConverter.ToInt64(buffer, j);
                _Mtime = DateTime.FromFileTimeUtc(z);
                j += 8;
                ...
                _Atime = DateTime.FromFileTimeUtc(z);
                j += 8;

                z = BitConverter.ToInt64(buffer, j);
                _Ctime = DateTime.FromFileTimeUtc(z);
                j += 8;
