...
namespace IgxlData.Zip
{
    ...
    ///         file. All of these things are done automatically by a call to
    ///         <see
    ///             cref="ZipEntry.Extract()">
    ///             ZipEntry.Extract()
    ///         </see>
    ///         .  For this reason, the
    ...
    internal class ZipInputStream : Stream
    {
        ...
        ///             <see
        ///                 cref="IgxlData.PZipOutputStreamStream" />
        ///             .
        ///         </item>
        ///         <item>
        ///             <c>ZipInputStream</c> cannot read segmented or spanned
        ...
        ///         entry is encrypted and requires a password, check the
        ///         <see
        ///             cref="ZipEntry.Encryption">
        ///             ZipEntry.Encryption
        ///         </see>
        ///         property.
        ///     </para>
        ...
        ///         stream is moved to the next entry in the zip file.  If you call
        ///         <see
        ///             cref="Seek(long, SeekOrigin)" />
        ///         , and thus re-position the pointer within
        ...
        public ZipEntry GetNextEntry()
        {
            if (_findRequired)
            {
                ...
                var d = SharedUtilities.FindSignature(ReadStream, ZipConstants.ZipEntrySignature);
                if (d == -1) return null;
                // back up 4 bytes: ReadEntry assumes the file pointer is positioned before the entry signature
                ReadStream.Seek(-4, SeekOrigin.Current);
                // workitem 10178
                SharedUtilities.Workaround_Ladybug318918(ReadStream);
            }
            // workitem 10923
        ...
        ///     </para>
        ///     <para>
        ///         Applications can intermix calls to <c>Seek()</c> with calls to
        ///         <see
        ///             cref="GetNextEntry()" />
        ///         .  After a call to <c>Seek()</c>,
        ...
        {
            _findRequired = true;
            var x = ReadStream.Seek(offset, origin);
            // workitem 10178
            SharedUtilities.Workaround_Ladybug318918(ReadStream);
            return x;
        }
