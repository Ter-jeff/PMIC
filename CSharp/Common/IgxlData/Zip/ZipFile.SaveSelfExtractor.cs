...
namespace IgxlData.Zip
{
    ...
    internal class SelfExtractorSaveOptions
    {
        ...
        ///         just as you would on a normal command line. Example:
        ///         <c>
        ///             program.exe arg1
        ///             arg2
        ///         </c>
        ///         .  The string prior to the first space will be taken as the
        ...
        ///         the the user's personal directory (
        ///         <see
        ///             cref="Environment.SpecialFolder.Personal" />
        ///         ) for the default extract
        ///         location.
        ///     </para>
        ///     <para>
        ...
        ///                 </para>
        ///                 <para>
        ///                     Double-clicking the SFX in Windows
        ...
        ///             PostExtractCommandLine
        ///         </see>
        ///         , and if the command runs successfully,
    ...
    public partial class ZipFile
    {
        ...
        //         "namespace IgxlData.Packaging.DotNetZip { internal partial class WinFormsSelfExtractorStub { partial void _SetDefaultExtractLocation() {" +
        ...
        ///         to by <see cref="ZipFile.TempFileFolder" />, which defaults to
        ///         <see
        ///             cref="System.IO.Path.GetTempPath" />
        ...
        private static void ExtractResourceToFile(Assembly a, string resourceName, string filename)
        {
            ...
            using (var instream = a.GetManifestResourceStream(resourceName))
            {
                if (instream == null)
        ...
        private void _SaveSfxStub(string exeToGenerate, SelfExtractorSaveOptions options)
        {
            ...
            try
            {
                if (File.Exists(exeToGenerate))
                    if (Verbose)
                        StatusMessageTextWriter.WriteLine("The existing file ({0}) will be overwritten.",
                            exeToGenerate);
                ...
                var a1 = typeof(ZipFile).Assembly;
                           ...
                           (new Dictionary<string, string> {{"CompilerVersion", "v2.0"}}))
                {
                    ...
                    if (settings == null)
                        throw new BadStateException(string.Format(
                            "While saving a Self-Extracting Zip, Cannot find that flavor ({0})?", options.Flavor));
                    ...
                    if (settings.ReferencedAssemblies != null)
                        foreach (var ra in settings.ReferencedAssemblies)
                    ...
                    var sourceFile = GenerateTempPathname(tmpDir, "cs");
                    ...
                    using (var zip = Read(a2.GetManifestResourceStream("Ionic.Zip.Resources.ZippedResources.zip")))
                    {
                        ...
                        if (settings.CopyThroughResources != null && settings.CopyThroughResources.Count != 0)
                        {
                            ...
                            foreach (var re in settings.CopyThroughResources)
                            {
                                var filename = Path.Combine(unpackedResourceDir, re);
                        ...
                        var extractLoc = options.DefaultExtractDirectory;
                        if (extractLoc != null)
                        ...
                        var postExCmdLine = options.PostExtractCommandLine;
                        ...
                        foreach (var rc in settings.ResourcesToCompile)
                            using (Stream s = zip[rc].OpenReader())
                            {
                                ...
                                using (var sr = new StreamReader(s))
                                {
                                    while (sr.Peek() >= 0)
                                    {
                                        ...
                                        line = line.Replace("@@REMOVE_AFTER_EXECUTE",
                                            options.RemoveUnpackedFilesAfterExecute.ToString());
                                        ...
                                        line = line.Replace("@@EXTRACT_EXISTING_FILE",
                                            ((int) options.ExtractExistingFile).ToString());
                    ...
                    if (cr.Errors.Count != 0)
                    {
                        using (TextWriter tw = new StreamWriter(sourceFile))
                        {
                            ...
                            var p = Path.GetFileName(sourceFile);

                            foreach (CompilerError error in cr.Errors)
                                tw.Write("//   {0}({1},{2}): {3} {4}: {5}\n//\n", p, error.Line, error.Column,
                                    error.IsWarning ? "Warning" : "error", error.ErrorNumber, error.ErrorText); // 5
                        }

                        throw new SfxGenerationException(string.Format("Errors compiling the extraction logic!  {0}",
                            sourceFile));
                    }
