...
namespace IgxlData.Zip
{
    public partial class ZipEntry
    {
        internal void WriteCentralDirectoryEntry(Stream s)
        {
            ...
            var extraFieldLength = (short) (_Extra == null ? 0 : _Extra.Length);
            bytes[i++] = (byte) (extraFieldLength & 0x00FF);
            bytes[i++] = (byte) ((extraFieldLength & 0xFF00) >> 8);

            // File (entry) Comment Length
            // the _CommentBytes private field was set during WriteHeader()
            var commentLength = _CommentBytes == null ? 0 : _CommentBytes.Length;
            ...
            // Disk number start
            var segmented = _container.ZipFile != null &&
            ...
            // internal file attrs
            // workitem 7801
            bytes[i++] = (byte) (IsText ? 1 : 0); // lo bit: filetype hint.  0=bin, 1=txt.
            bytes[i++] = 0;
        ...
        private void MaybeUnsetCompressionMethodForWriting(int cycle)
        {
            ...
            if (_Source == ZipEntrySource.ZipFile) return; // do nothing
        ...
        internal void WriteHeader(Stream s, int cycle)
        {
            ...
            if (zss != null)
            {
                ...
                var requiredSegment = zss.ComputeSegment(i);
                ...
                if (_Password != null)
                {
                    ...
                    if (Encryption == EncryptionAlgorithm.PkzipWeak)
                        headerBytesToRetract = 12;
                             ...
                             Encryption == EncryptionAlgorithm.WinZipAes256)
                    {
                        headerBytesToRetract =
 _aesCrypto_forWrite._Salt.Length + _aesCrypto_forWrite.GeneratedPV.Length;
                    }
#endif
            ...
            // (i==26) filename length (Int16)
            var filenameLength = (short) (_EntryHeader[26] + _EntryHeader[27] * 256);
            var extraFieldLength = (short) (_EntryHeader[28] + _EntryHeader[29] * 256);
            ...
            else
            {
                ...
                if (extraFieldLength != 0)
                {
                    ...
                    var DataSize = (short) (_EntryHeader[i + 2] + _EntryHeader[i + 3] * 256);
                    if (DataSize == 16)
                    {
                ...
                (_Source == ZipEntrySource.ZipOutputStream && s.CanSeek))
            {
                ...
                if (zss != null && _diskNumber != zss.CurrentSegment)
                {
                    ...
                    using (var hseg = ZipSegmentedStream.ForUpdate(_container.ZipFile.Name, _diskNumber))
                    {
            ...
            {
                var Descriptor = new byte[16 + (_OutputUsesZip64.Value ? 8 : 0)];
                i = 0;
        ...
        private Stream MaybeApplyCompression(Stream s, long streamLength)
        {
            if (_CompressionMethod == 0x08 && CompressionLevel != CompressionLevel.None)
            {
                     ...
                     _container.ParallelDeflateThreshold > 0L))
                {
                    ...
                    var o1 = _container.ParallelDeflater;
                    o1.Reset(s);
                    return o1;
                }
#endif
        ...
        internal void WriteSecurityMetadata(Stream outstream)
        {
            ...
            if (Encryption == EncryptionAlgorithm.PkzipWeak)
            {
                ...
                var rnd = new Random();
                var encryptionHeader = new byte[12];
                ...
                var cipherText = _zipCrypto_forWrite.EncryptMessage(encryptionHeader, encryptionHeader.Length);
        ...
        private void CopyThroughOneEntry(Stream outStream)
        {
            ...
            var needRecompute = _metadataChanged ||
        ...
        private void CopyThroughWithRecompute(Stream outstream)
        {
            ...
            var input = new CountingStream(ArchiveStream);

            var origRelativeOffsetOfHeader = _RelativeOffsetOfLocalHeader;

            // The header length may change due to rename of file, add a comment, etc.
            // We need to retain the original.
            var origLengthOfHeader = LengthOfHeader; // including crypto bytes!
            ...
            if (!FileName.EndsWith("/"))
            {
                ...
                var pos = origRelativeOffsetOfHeader + origLengthOfHeader;
                var len = GetLengthOfCryptoHeaderBytes(_Encryption_FromZipFile);
        ...
        private void CopyThroughWithNoChange(Stream outstream)
        {
            ...
            var remaining = _TotalEntrySize;
            while (remaining > 0)
            {
                var len = remaining > bytes.Length ? bytes.Length : (int) remaining;
