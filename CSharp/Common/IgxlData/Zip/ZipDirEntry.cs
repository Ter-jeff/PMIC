...
namespace IgxlData.Zip
{
    public partial class ZipEntry
    {
        ...
        {
            var s = zf.ReadStream;
            var expectedEncoding = zf.AlternateEncodingUsage == ZipOption.Always
            ...
            unchecked
            {
                ...
                zde._CompressedSize = (uint) (block[i++] + block[i++] * 256 + block[i++] * 256 * 256 +
                                              block[i++] * 256 * 256 * 256);
                zde._UncompressedSize = (uint) (block[i++] + block[i++] * 256 + block[i++] * 256 * 256 +
                                                block[i++] * 256 * 256 * 256);
            }
            ...
            zde._RelativeOffsetOfLocalHeader = (uint) (block[i++] + block[i++] * 256 + block[i++] * 256 * 256 +
                                                       block[i++] * 256 * 256 * 256);
        ...
        private class CopyHelper
        {
            ...
            internal static string AppendCopyToFileName(string f)
            {
                ...
                else
                {
                    ...
                    var m = re.Match(f.Substring(0, r));
                    if (m.Success)
                    {
                        n = int.Parse(m.Groups[1].Value) + 1;
                        var copy = string.Format(" (copy {0})", n);
