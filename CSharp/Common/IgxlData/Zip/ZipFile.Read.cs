...
namespace IgxlData.Zip
{
    ...
    public partial class ZipFile
    {
            ...
            EventHandler<ReadProgressEventArgs> readProgress)
        {
            ...
            if (zf.Verbose) zf.StatusMessageTextWriter.WriteLine("reading from {0}...", fileName);
            ...
            EventHandler<ReadProgressEventArgs> readProgress)
        {
            ...
            if (zf.Verbose) zf.StatusMessageTextWriter.WriteLine("reading from stream...");
        ...
        private static void ReadIntoInstance(ZipFile zf)
        {
            ...
            try
            {
                zf._readName = zf.Name; // workitem 13915
                if (!s.CanSeek)
                {
                ...
                var datum = ReadFirstFourBytes(s);
                ...
                var posn = s.Length - 64;
                var maxSeekback = Math.Max(s.Length - 0x4000, 10);
                do
                {
                    if (posn < 0) posn = 0; // BOF
                    s.Seek(posn, SeekOrigin.Begin);
                    var bytesRead = SharedUtilities.FindSignature(s, (int) ZipConstants.EndOfCentralDirectorySignature);
                    if (bytesRead != -1)
                    {
                        success = true;
                    }
                ...
                if (success)
                {
                    ...
                    var offset32 = BitConverter.ToUInt32(block, i);
        ...
        private static void Zip64SeekToCentralDirectory(ZipFile zf)
        {
            ...
            if (datum != ZipConstants.Zip64EndOfCentralDirectoryRecordSignature)
                throw new BadReadException(string.Format(
                    "  Bad signature (0x{0:X8}) looking for ZIP64 EoCD Record at position 0x{1:X8}", datum,
                    s.Position));
        ...
        private static void ReadIntoInstance_Orig(ZipFile zf)
        {
            ...
            try
            {
                ...
                while ((de = ZipEntry.ReadDirEntry(zf, previouslySeen)) != null)
                {
                    ...
                    var e1 = zf._entries[de.FileName];
                    if (e1 != null)
                    {
        ...
        private static void ReadZipFileComment(ZipFile zf)
        {
            ...
            if (commentLength > 0)
            {
                ...
                var s1 = zf.AlternateEncoding.GetString(block, 0, block.Length);
                zf.Comment = s1;
            }
        }
