...
namespace IgxlData.Zip
{
    ...
    internal static class SharedUtilities
    {
        ...
        public static DateTime RoundToEvenSecond(DateTime source)
        {
            ...
            DateTime dtRounded =
 new DateTime(source.Year, source.Month, source.Day, source.Hour, source.Minute, source.Second);
        ...
        private static readonly Regex doubleDotRegex1 =
            new Regex(@"^(.*/)?([^/\\.]+/\\.\\./)(.+)$");
        ...
        internal static DateTime PackedToDateTime(int packedDateTime)
        {
            ...
            if (!success)
            {
                var msg = string.Format("y({0}) m({1}) d({2}) h({3}) m({4}) s({5})", year, month, day, hour, minute,
                    second);
            ...
            static int DateTimeToPacked(DateTime time)
        {
            ...
            var packedDate = (ushort) ((time.Day & 0x0000001F) | ((time.Month << 5) & 0x000001E0) |
                                       (((time.Year - 1980) << 9) & 0x0000FE00));
            var packedTime = (ushort) (((time.Second / 2) & 0x0000001F) | ((time.Minute << 5) & 0x000007E0) |
                                       ((time.Hour << 11) & 0x0000F800));
    ...
    internal class CountingStream : Stream
    {
        ...
        {
            get { return _initialOffset + BytesWritten; }
        }
        ...
        public override long Position
        {
            ...
            set
            {
                WrappedStream.Seek(value, SeekOrigin.Begin);
                // workitem 10178
                SharedUtilities.Workaround_Ladybug318918(WrappedStream);
            }
        }
        ...
        public void Adjust(long delta)
        {
            ...
            if (WrappedStream as CountingStream != null)
                ((CountingStream) WrappedStream).Adjust(delta);
        }
        ...
        {
            var n = WrappedStream.Read(buffer, offset, count);
            BytesRead += n;
            return n;
        }
