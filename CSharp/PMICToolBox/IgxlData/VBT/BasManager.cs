...
namespace IgxlData.VBT
{
    ...
    {
        public List<VbtFunctionBase> VbtFunctionBases;

        public BasManager(string tempPath)
        {
            VbtFunctionBases = ReadAllLib(tempPath);
        }

        private List<VbtFunctionBase> ReadAllLib(string dirLib)
        {
        ...
        public static List<Proc> GetProcOfLine(string fileName)
        {
        ...
        public static List<string> GetFuncFrame(string file, string functionName, bool ignoreContent = true, bool ignoreFrame = false)
        {
            var funcFrame = new List<string>();
            var content = ReadBasContent(file);
            var tarline = SearchContent(content, new List<string>
            {
                " Function " + functionName
            });

            if (!string.IsNullOrEmpty(tarline))
            {
                int index = content.IndexOf(tarline);
                bool pass = false;
                while (index < content.Count && !content[index].ToLower().Contains("end function"))
                {
                    if (!pass)
                    {
                        funcFrame.Add(content[index]);
                        index++;

                        if ((Regex.IsMatch(content[index], @".*\)") || Regex.IsMatch(content[index], @"\(.*\)")) && ignoreContent)
                        {
                            pass = true;
                            funcFrame.Add(content[index]);
                        }
        ...
        private static List<Parameter> GetParamters(string paramStr)
        {
        ...
        private static string GetFunctionName(string line, ref string type)
        {
        ...
        public VbtFunctionBase GetFunctionByName(string functionName)
        {
            ...
            if (VbtFunctionBases == null)
                return resultVbt;

            var newVbt = VbtFunctionBases.Find(a => a.FunctionName.ToLower() == functionName.ToLower());
            if (newVbt == null)
                return resultVbt;
