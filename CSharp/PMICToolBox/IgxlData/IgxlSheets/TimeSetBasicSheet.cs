...
namespace IgxlData.IgxlSheets
{
    ...
    public class TimeSetBasicSheet : IgxlSheet
    {
        ...
        public TimeSetBasicSheet(ExcelWorksheet sheet, string timingMode, string masterTimeSet, string timeDomain, string strobeRefSetup)
            : base(sheet)
        {
            Tsets = new List<Tset>();
            TimingMode = timingMode;
        ...
        public TimeSetBasicSheet(string sheetName, string timingMode, string masterTimeSet, string timeDomain, string strobeRefSetup)
            : base(sheetName)
        {
            Tsets = new List<Tset>();
            TimingMode = timingMode;
        ...
        //{
        //    const string header = "DTTimesetBasicSheet,version=1.4:platform=Jaguar:toprow=-1:leftcol=-1:rightcol=-1\tTime Sets (Basic)\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t";
        ...
        //{
        //    const string columnsName = "\t\tCycle\tPin Group\t\t\tData\t\tDrive\t\t\t\tCompare\t\t\tEdge Resolution\t\t";
        //    IgxlWriter.WriteLine(columnsName);
        //    IgxlWriter.WriteLine("\tTime Set\tPeriod\tName\tClock Period\tSetup\tSrc\tFmt\tOn\tData\tReturn\tOff\tMode\tOpen\tClose\tMode\tComment\t");
        //}
        ...
        public override void Write(string fileName, string version = "2.3")
        {
            //if (version == "1.4")
            //{
            ...
            //    GenTSBSheet genTimeSet = new GenTSBSheet(fileName, TimingMode, validate, true);


            //    foreach (var tsb in Tsets)
            //    {
            //        foreach (var timingRow in tsb.TimingRows)
            //        {
            //            genTimeSet.AddTimeSet(tsb.Name, tsb.CyclePeriod, timingRow.PinGrpName, timingRow.PinGrpClockPeriod, timingRow.PinGrpSetup
            //                , timingRow.DataSrc, timingRow.DataFmt, timingRow.DriveOn, timingRow.DriveData, timingRow.DriveReturn, timingRow.DriveOff
            //                , timingRow.CompareMode, timingRow.CompareOpen, timingRow.CompareClose, timingRow.CompareRefOffset);
            //        }
            //    }
            //    genTimeSet.WriteSheet();
            //    //throw new Exception(string.Format("The TimeSet Basic sheet version:{0} is not supported!"));
            //}
            //else
            //{
                    ...
                    if (igxlSheetsVersion.Field != null)
                    {
                        foreach (var item in igxlSheetsVersion.Field)
                        {
                            if (item.rowIndex == i)
                            {
                                ...
                                if (item.fieldName.Equals("Master Timeset Name:", StringComparison.CurrentCultureIgnoreCase))
                                ...
                                if (item.fieldName.Equals("Strobe Ref Setup Name:", StringComparison.CurrentCultureIgnoreCase))
