...
namespace IgxlData.Others
{
    public class SpecFinder
    {
        ...
        public SpecFinder(List<GlobalSpecSheet> globalSpecsSheets, List<AcSpecSheet> acSpecSheets, List<DcSpecSheet> dcSpecSheets, List<LevelSheet> levelSheets)
        {
            _acSpecSheets = acSpecSheets;
            _dcSpecSheets = dcSpecSheets;
            _levelSheets = levelSheets;
            _globalSpecs = globalSpecsSheets.Where(x => x != null).SelectMany(x => x.Specs).ToList();
        ...
        public string GetValue(InstanceRow instanceRow, string formula, string frequencyName)
        {
            ...
            return GetFomulaValue(formula, _globalSpecs);
        }

        public string GetValue(InstanceRow instanceRow, string formula)
        {
            if (instanceRow == null) return "";
            if (_levelSheets.Exists(x => x.Name.Equals(instanceRow.PinLevels, StringComparison.CurrentCultureIgnoreCase)))
            {
                var levelSheet = _levelSheets.Find(x => x.Name.Equals(instanceRow.PinLevels, StringComparison.CurrentCultureIgnoreCase));
                if (levelSheet.LevelRows.Exists(x => x.PinName.Equals(formula, StringComparison.CurrentCultureIgnoreCase)))
                {
                    var row = levelSheet.LevelRows.Where(x => x.PinName.Equals(formula, StringComparison.CurrentCultureIgnoreCase))
                        .First(x => x.Parameter.Equals("Vps", StringComparison.CurrentCultureIgnoreCase) || x.Parameter.Equals("Vmain", StringComparison.CurrentCultureIgnoreCase));
        ...
        private string GetFomulaValue(string formula, List<GlobalSpec> globalSpecs)
        {
            try
            {
        ...
        private string ReplaceGlobalValue(string formula, List<GlobalSpec> globalSpecs)
        {
            ...
            foreach (var item in Regex.Matches(formula, regex, RegexOptions.IgnoreCase))
            {
                ...
                {
                    var row = globalSpecs.Find(x => item.ToString().Equals("_" + x.Symbol, StringComparison.OrdinalIgnoreCase));
