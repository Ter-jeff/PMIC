...
namespace IgxlData.Others.PatternListCsvFile
{
    public class PatternNameInfo
    {
        ...
        {
            get { return string.Format("{0}_{1}_{2}", PatternVersion, SiliconVersion, TimeStamp); }
        }

        //抽取資訊的Method寫在父層 勢必已經檢查完符合Naming Rule
        public PatternNameInfo(string patName, string regdateCode = @"\d{10}")
        {
            this._regDateCode = regdateCode;
            ...
            //竟然發現有如下的奇葩
            //  ***** List Pattern Start ******
            //  Pattern : .\Pattern\SOC_SCAN\PP_MLA_S_IN00_SC_CCC0_SAA_COM_AUT_ALLFV_DM_1_A0_1501112122.PAT
            //  Pattern : ./Pattern/SOC_SCAN/PP_MLA_S_PL00_SC_CCC0_SAA_COM_AUT_ALLFV_DM_3_A0_1502111658.pat
            //***** List Pattern end ******
            ...
            var regPattern = string.Format(@"(?<name>.*_{0})", _regDateCode);
            if (Regex.IsMatch(patName, regPattern))
            {
                ...
                if (Regex.IsMatch(TimeStamp, _regDateCode) || Regex.IsMatch(TimeStamp, @"YYMMDDHHMM"))
                {
                ...
                GenericName = patName; //For DFT Pattern List xxxx_<>
        }
