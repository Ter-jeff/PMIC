...
namespace IgxlData.Others
{
    public class DivideFlowMain
    {
        #region Field
        private int _cnt;
        private const int MaxDivedeRow = 100000 / 2;
        ...
        public Dictionary<string, SubFlowSheet> WorkFlow(Dictionary<string, SubFlowSheet> subFlowSheets)
        {
            ...
            {
                _cnt = 0;
                if (flow.Value.FlowRows.Count > MaxDivedeRow)
                {
        ...
        private KeyValuePair<string, SubFlowSheet> DivideFlow(KeyValuePair<string, SubFlowSheet> flow, int index)
        {
            ...
            {
                _cnt = _cnt + 1;
                int location = flow.Value.Name.IndexOf("Part", StringComparison.Ordinal);
                string newSheetName = location == -1 ? flow.Value.Name + "_Part" + _cnt : flow.Value.Name.Substring(0, location) + "Part" + _cnt;
        ...
        private int GetDivideIndex(KeyValuePair<string, SubFlowSheet> flow)
        {
            ...
            {
                if (Regex.IsMatch(item.opcode, "test|if|for", RegexOptions.IgnoreCase) & item.index > MaxDivedeRow &
