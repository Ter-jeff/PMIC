...
namespace PmicAutogen.Inputs.TestPlan.Reader.DcTestConti
{
    public class DcTestContinuityReader
    {
        ...
        private void GetDimension()
        {
            ...
            _endColNum = _excelWorksheet.Dimension.End.Column;
            _endRowNum = _excelWorksheet.Dimension.End.Row;
        }
        ...
        private void ReadHeader()
        {
            ...
            {
                var header = _excelWorksheet.GetCellValue(_startRowNum, i);
        ...
        private DcTestContinuitySheet ReadData()
        {
            ...
            var mergeCellCnt = 2;
            var range = _excelWorksheet.MergedCells[_startRowNum, _categoryIndex];
            if (range != null)
            {
            ...
            for (var i = _startRowNum + mergeCellCnt; i <= _endRowNum; i++)
            {
                ...
                dcTestContiRow.Category = _excelWorksheet.GetMergedCellValue(i, _categoryIndex).Trim();
                dcTestContiRow.PinGroup = GetCellValue(i, _pinGroupIndex).Replace(" ", "_");
                if (_timeSetIndex != -1)
                    dcTestContiRow.TimeSet = GetCellValue(i, _timeSetIndex).Trim();
                dcTestContiRow.Condition = _excelWorksheet.GetMergedCellValue(i, _conditionIndex);
                ...
                for (var j = 0; j < _limitValueNum; j += 4)
                {
                    ...
                    if (_excelWorksheet.GetMergedCellValue(row, col).Contains(DcTestContiRow.ConHeaderHiShort))
                        dcTestContiSheetLimit.ShortHiLimitValue = _excelWorksheet.GetMergedCellValue(i, col);
                    if (_excelWorksheet.GetMergedCellValue(row, col + 1)
                        .Contains(DcTestContiRow.ConHeaderLoShort))
                        dcTestContiSheetLimit.ShortLoLimitValue =
                            _excelWorksheet.GetMergedCellValue(i, col + 1);
                    if (_excelWorksheet.GetMergedCellValue(row, col + 2).Equals(DcTestContiRow.ConHeaderHiOpen))
                        dcTestContiSheetLimit.OpenHiLimitValue = _excelWorksheet.GetMergedCellValue(i, col + 2);
                    if (_excelWorksheet.GetMergedCellValue(row, col + 3).Equals(DcTestContiRow.ConHeaderLoOpen))
                        dcTestContiSheetLimit.OpenLoLimitValue = _excelWorksheet.GetMergedCellValue(i, col + 3);
        ...
        {
            var range = _excelWorksheet.MergedCells[pIRow, pIColumn];
            if (range != null)
            {
                var lAddress = new ExcelAddress(range);
