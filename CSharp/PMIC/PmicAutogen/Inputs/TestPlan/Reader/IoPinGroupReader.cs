        ...
        private IoPinGroupSheet ReadSheetData()
        {
            ...
            for (var i = _startRowNumber + 1; i <= _endRowNumber; i++)
            {
                ...
                if (_pinGroupNameIndex != -1)
                    row.PinGroupName = _excelWorksheet.GetMergedCellValue(i, _pinGroupNameIndex)
                        .Trim();
                if (_pinNameContainedInPinGroupIndex != -1)
                    row.PinNameContainedInPinGroup = _excelWorksheet.GetMergedCellValue(i, _pinNameContainedInPinGroupIndex).Trim();
                if (_commentsIndex != -1)
                    row.Comments = _excelWorksheet.GetMergedCellValue(i, _commentsIndex).Trim();
