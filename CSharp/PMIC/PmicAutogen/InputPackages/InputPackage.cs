...
namespace PmicAutogen.InputPackages
{
    ...
    {
        public delegate void WriteMessage(string msg, EnumMessageLevel level = EnumMessageLevel.General, int percentage = -1);
        ...
        {
            //var groups = files.GroupBy(x => x);
            //foreach (var group in groups)
            //    if (group.Count() > 1)
            //    {
            //        Response.Report("[Duplicated] file already existed! " + group.Key, MessageLevel.Error, 0);
            //        return;
            //    }
            ...
            foreach (var file in files)
            {
                ...
                {
                    Response.Report("Unknown File Type: " + file, EnumMessageLevel.Error, 0);
                }
            }
        ...
        private InputFileType GetTxtFileType(string fileName)
        {
            ...
            if (Regex.IsMatch(bufferString,
                    @"IG-XL\s+Version[\s|\S+]*Test Program[\s|\S+]*Total Execution Time[\s|\S+]*",
        ...
        public List<InputOtpRegisterMap> GetSelectedOtpFile()
        {
            ...
            return otpOptionFile.OfType<InputOtpRegisterMap>().ToList();
