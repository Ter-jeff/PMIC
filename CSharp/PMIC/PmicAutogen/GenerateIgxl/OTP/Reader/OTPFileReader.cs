...
namespace PmicAutogen.GenerateIgxl.OTP.Reader
{
    public class OtpFileReader
    {
        ...
        public string VersionRawDataRow { get; private set; } = "";
        ...
        public void MergeToYaml(List<OtpRegister> otpRegisters, string otpFile)
        {
            ...
            if (infoSubset.Count < 2)
            {
                ...
                Response.Report("It is better to make your .OTP input file naming as below : ", EnumMessageLevel.Warning,
                    0);
                Response.Report("OTP/Yaml file name should be end with string like 'OTP_[version]",
                    EnumMessageLevel.Warning, 0);
            }

            if (infoSubset.Count >= 2)
            {
                var version = string.Join("_", infoSubset.GetRange(infoSubset.Count - 2, 2));
                if (!Regex.IsMatch(version, @"OTP_[\w]+", RegexOptions.IgnoreCase))
                    //MessageBox.Show("OTP/Yaml file name should be end with string like 'OTP_[version]", "Warning", MessageBoxButtons.OK);
                    Response.Report("It is better to make your .OTP input file naming as below : ",
                        EnumMessageLevel.Warning, 0);
                Response.Report("OTP/Yaml file name should be end with string like 'OTP_[version]",
                    EnumMessageLevel.Warning, 0);
                Headers.Add(version);
