...
namespace PmicAutogen.GenerateIgxl.Basic.GenPatSet
{
    public class PatSetGenerator
    {
        ...
        public void GenerateFlow(List<PatternListCsvRow> patternListCsvRows, string patternPath)
        {
            ...
            foreach (var item in _patSetSheetAllNoPat.PatSets)
                PatSetSheetAll.AddPatSet(item);

            if (_patSubSheetAllNoPat.PatSetSubRows.Count > 0)
            {
                var dummyPatSub = new PatSetSubRow { PatternFileName = "" };
                PatSubSheetAll.AddRow(dummyPatSub);
                PatSubSheetAll.AddRow(dummyPatSub);
                foreach (var item in _patSubSheetAllNoPat.PatSetSubRows) PatSubSheetAll.AddRow(item);
            }

            if (PatSubSheetAll.PatSetSubRows.Count == 0)
                PatSubSheetAll = null;
        }
