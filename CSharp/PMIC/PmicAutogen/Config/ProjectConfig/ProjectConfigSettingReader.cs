    ...
    public class ProjectConfigSettingReader
    {
        ...
        private ProjectConfigSettingSheet ReadSheetData()
        {
            for (var i = _startRowNumber + 1; i <= _endRowNumber; i++)
            {
                ...
                row.RowNum = i;
                if (_nameIndex != -1)
                    row.Name = _excelWorksheet.GetMergedCellValue(i, _nameIndex).Trim();
                if (_groupNameIndex != -1)
                    row.GroupName = _excelWorksheet.GetMergedCellValue(i, _groupNameIndex).Trim();
                if (_deviceIndex != -1)
                    row.TabGroup = _excelWorksheet.GetMergedCellValue(i, _deviceIndex).Trim();
                if (_descriptionIndex != -1)
                    row.Description = _excelWorksheet.GetMergedCellValue(i, _descriptionIndex).Trim();
                if (_typeIndex != -1)
                    row.Type = _excelWorksheet.GetMergedCellValue(i, _typeIndex).Trim();
                if (_maxlengthIndex != -1)
                {
                    int value;
                    int.TryParse(_excelWorksheet.GetMergedCellValue(i, _maxlengthIndex).Trim(),
                        out value);
                    row.MaxLength = value;
                }

                if (_defaultIndex != -1)
                    row.Default = _excelWorksheet.GetMergedCellValue(i, _defaultIndex).Trim();
                if (_optionValue1Index != -1)
                    row.OptionValue.Add(_excelWorksheet.GetMergedCellValue(i, _optionValue1Index)
                        .Trim());
                if (_optionValue2Index != -1)
                    row.OptionValue.Add(_excelWorksheet.GetMergedCellValue(i, _optionValue2Index)
                        .Trim());
                if (_optionValue3Index != -1)
                    row.OptionValue.Add(_excelWorksheet.GetMergedCellValue(i, _optionValue3Index)
                        .Trim());
                if (_optionValue4Index != -1)
                    row.OptionValue.Add(_excelWorksheet.GetMergedCellValue(i, _optionValue4Index)
                        .Trim());
                if (_optionValue5Index != -1)
                    row.OptionValue.Add(_excelWorksheet.GetMergedCellValue(i, _optionValue5Index)
                        .Trim());
                if (_optionValue6Index != -1)
                    row.OptionValue.Add(_excelWorksheet.GetMergedCellValue(i, _optionValue6Index)
                        .Trim());
                if (_optionValue7Index != -1)
                    row.OptionValue.Add(_excelWorksheet.GetMergedCellValue(i, _optionValue7Index)
                        .Trim());
                if (_optionValue8Index != -1)
                    row.OptionValue.Add(_excelWorksheet.GetMergedCellValue(i, _optionValue8Index)
                        .Trim());
                if (_optionValue9Index != -1)
                    row.OptionValue.Add(_excelWorksheet.GetMergedCellValue(i, _optionValue9Index)
                        .Trim());
                if (_optionValue10Index != -1)
                    row.OptionValue.Add(_excelWorksheet.GetMergedCellValue(i, _optionValue10Index)
                        .Trim());
