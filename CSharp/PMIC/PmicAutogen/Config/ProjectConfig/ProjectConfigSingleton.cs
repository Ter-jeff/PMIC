    ...
    public class ProjectConfigSingleton
    {
        ...
        public void LoadProjectConfig()
        {
            ...
            if (InputFiles.InteropTestPlanWorkbook != null)
                worksheet = InputFiles.InteropTestPlanWorkbook.GetSheet(PmicConst.ProjectConfig);
            ...
            {
                var sheet = InputFiles.TestPlanWorkbook.Worksheets[PmicConst.ProjectConfig];
                if (sheet != null)
                {
                    var projectConfigReader = new ProjectConfigEpplusReader();
                    var projectConfigSheet = projectConfigReader.ReadSheet(sheet);
                    _instance._projectConfigRows = projectConfigSheet.Rows;
                }
            }
        }
        ...
        public string ReplaceItemNameByConfigGroup(string group, string source)
        {
            ...
            if (values.Count > 0)
            {
                foreach (var value in values)
                    result = Regex.Replace(source, value.Name, value.Value, RegexOptions.IgnoreCase);
