...
namespace PmicAutogen.Config.ProjectConfig
{
    ...
    public class ProjectConfigEpplusReader : MySheetReader
    {
        private const string HeaderName = "Name";
        private const string HeaderGroupName = "GroupName";
        private const string HeaderValue = "Value";
        ...
        public ProjectConfigSheet ReadSheet(ExcelWorksheet worksheet)
        {
            if (!ReadHeaders(worksheet)) return new ProjectConfigSheet();
        ...
        private bool ReadHeaders(ExcelWorksheet worksheet)
        {
            if (worksheet == null) return false;
        ...
        private ProjectConfigSheet ReadRows()
        {
            ...
            for (var i = StartRowNumber + 1; i <= EndRowNumber; i++)
            {
                var row = new ProjectConfigRow();
                if (_nameIndex != -1)
                    row.Name = ExcelWorksheet.GetMergedCellValue(i, _nameIndex).Trim();
                if (_groupNameIndex != -1)
                    row.GroupName = ExcelWorksheet.GetMergedCellValue(i, _groupNameIndex).Trim();
                if (_valueIndex != -1)
                    row.Value = ExcelWorksheet.GetMergedCellValue(i, _valueIndex).Trim();
                projectConfigSheet.Rows.Add(row);
            }
        ...
        private void GetHeaderIndex()
        {
            ...
            {
                string lStrHeader = ExcelWorksheet.GetMergedCellValue(StartRowNumber, i).Trim();
                if (lStrHeader.Equals(HeaderName, StringComparison.OrdinalIgnoreCase))
                {
                    _nameIndex = i;
                    continue;
        ...
        private bool GetFirstHeaderPosition()
        {
            ...
            for (var i = 1; i <= rowNum; i++)
                for (var j = 1; j <= colNum; j++)
                    if (ExcelWorksheet.GetMergedCellValue(i, j).Trim()
                        .Equals(HeaderName, StringComparison.OrdinalIgnoreCase))
                    {
                        StartRowNumber = i;
                        return true;
