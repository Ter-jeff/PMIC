        ...
        private SheetConfigSheet ReadSheetData()
        {
            for (var i = _startRowNumber + 1; i <= _endRowNumber; i++)
            {
                ...
                if (_sheetNameIndex != -1)
                    row.SheetName = _excelWorksheet.GetMergedCellValue(i, _sheetNameIndex).Trim();
                if (_firstHeaderNameIndex != -1)
                    row.FirstHeaderName = _excelWorksheet.GetMergedCellValue(i, _firstHeaderNameIndex)
                        .Trim();
                if (_headerNameIndex != -1)
                    row.HeaderName = _excelWorksheet.GetMergedCellValue(i, _headerNameIndex).Trim();
                if (_optionalIndex != -1)
                    row.Optional = _excelWorksheet.GetMergedCellValue(i, _optionalIndex).Trim();
                if (_typeIndex != -1)
