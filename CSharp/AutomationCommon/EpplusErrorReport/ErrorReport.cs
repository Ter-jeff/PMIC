...
namespace AutomationCommon.EpplusErrorReport
{
    public class ErrorReport
    {
        ...
        public void WriteReport(ExcelWorkbook workbook, string summaryReport = "SummaryReport")
        {
            if (_errors.Count == 0)
                return;

            var finalErrorReportName = ReportName;

            try
            {
                List<string> names = workbook.Worksheets.Select(x => x.Name).ToList();
                List<Error> errorsNeedToWrite = _errors.Where(x =>
                    ...
                    .ToList();
                WriteErrors(workbook, errorsNeedToWrite, finalErrorReportName, summaryReport);
        ...
        public void WriteReport(List<ExcelWorkbook> workbooks, string errorReportName = "", string summaryReport = "SummaryReport")
        {
            ...
            var finalErrorReportName = !string.IsNullOrEmpty(errorReportName) ? errorReportName : ReportName;
        ...
        private void WriteErrors(ExcelWorkbook workbook, List<Error> errors, string reportName, string summaryReport = "SummaryReport")
        {
            ...
            foreach (string errorType in typeList)
            {
                ...
                object[] subSumRow = { errorType, "", "", "", "", "", "", subErrors.Count };
        ...
        private void AppendSummaryReport(ExcelWorkbook workbook, string reportName, string summaryReport, int count)
        {
            ...
            int startRow = sheet.Dimension.End == null ? 1 : sheet.Dimension.End.Row + 1;
