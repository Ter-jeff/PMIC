...
namespace IgxlData.Others
{
    ...
    //    private Dictionary<string, Sheet.SheetTypes> _dicAllTxt;
    //    private readonly Regex _pattenRegex = new Regex(@"(\.\\(?:[^\\\?\/\*\|<>:]+\\)+)([^\\\?\/\*\|<>:]+?)\.(Pat|pat|PAT)", RegexOptions.Compiled | RegexOptions.IgnoreCase);

    //    private readonly List<PatternInTestProgram> _patternInTestPrograms;
    ...
    //        _dicAllTxt = new Dictionary<string, Sheet.SheetTypes>();
    ...
    //        var patSetSheets = (from pair in _dicAllTxt where pair.Value.ToString().Equals(Sheet.SheetTypes.DTPatternSetSheet.ToString()) select pair.Key).ToList();
    //        foreach (var sheet in patSetSheets)
    //        {
    //            var readPatSetSheet = new ReadPatSetSheet();
    //            var patSetSheet = readPatSetSheet.GetSheet(sheet);
    //            PatSetSheets.Add(patSetSheet.SheetName, patSetSheet);
    //        }
    ...
    //            {
    //                subFlowRows = subFlowRows.Where(x => x.IsMatchEnv(_env)).ToList();
    //                if (subFlowSheets.Exists(x => x.SheetName.Equals(_mainflow, StringComparison.CurrentCultureIgnoreCase)))
    //                {
    //                    var mainFlowRows = subFlowSheets.Find(x => x.SheetName.Equals(_mainflow, StringComparison.CurrentCultureIgnoreCase)).FlowRows
    ...
    //        var subFlow = (from pair in _dicAllTxt where pair.Value.ToString().Equals(Sheet.SheetTypes.DTFlowtableSheet.ToString()) select pair.Key).ToList();
    ...
    //    {
    //        var instanceFlow = (from pair in _dicAllTxt where pair.Value.ToString().Equals(Sheet.SheetTypes.DTTestInstancesSheet.ToString()) select pair.Key).ToList();
    //        var patternSetList = PatSetSheets.SelectMany(x => x.Value.PatSetRows).ToList();
    ...
    //                                RefreshPatternSecOrderFileName(instanceSheet.SheetName, instRow, item);
    //                        }
    //                    }
    //                }
    //            }
