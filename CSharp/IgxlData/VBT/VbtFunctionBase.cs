...
namespace IgxlData.VBT
{
    public class VbtFunctionBase
    {
        ...
        public string GetParamValue(string paramName)
        {
            string paramValue = "";
            int index = (Parameters.Split(',').ToList()).FindIndex(s => s.Equals(paramName, StringComparison.OrdinalIgnoreCase));
            var aliasList = VbtFunctionLib.ParamMappingList.FirstOrDefault(s => s.Find(a => a.Equals(paramName, StringComparison.OrdinalIgnoreCase)) != null);
            if (index != -1)
            ...
            else if (aliasList != null)
            {
                ...
                {
                    index = (Parameters.Split(',').ToList()).FindIndex(s => s.Equals(aliasName, StringComparison.OrdinalIgnoreCase));
                    if (index != -1)
                    {
                        paramValue = Args[index];
                        break;
