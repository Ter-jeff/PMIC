...
namespace PmicAutogen.GenerateIgxl.OTP.Writer
{
    public class WriterOTPSetup
    {
        ...
        public void outputOTPSetup(string path)
        {
            ...
            string startAddress = calculateStartAddress(_otpReader.VersionRawDataRow);
            string projectRevStr = readProjectRevStr();
            ...
            readRegisterNames(out otpEventFaultReg, out tpVersionMsb, out tpVersionLsb, out svnVersionMsb, out svnVersionLsb);
            using (StreamWriter writer = new StreamWriter(fileNameOTPSetup))
            {
                ...
                writer.WriteLine("TPVERSION_MSB\t" + tpVersionMsb + "\tRegister used to burn Test Program Version (MSB)");
                writer.WriteLine("TPVERSION_LSB\t" + tpVersionLsb + "\tRegister used to burn Test Program Version (LSB)");
                ...
                if (portMapSheet != null)
                {
                    ...
                    if (nwireJtagPortSetlist != null)
                    {
                        foreach (PortSet nwireJtagPortSet in nwireJtagPortSetlist)
                        {
                            ...
                            {
                                writer.WriteLine(string.Format("JTAG_{0}_Pin_Name\t{1}\tJTAG {2} Pin", row.FunctionName, row.FunctionPin, row.FunctionName));
                            }
                        }
                    }
                }
        ...
        {
            var validPatRow = InputFiles.PatternListMap.PatternListCsvRows.Find(s => s.PatternName.ToUpper().StartsWith("PP_") || s.PatternName.ToUpper().StartsWith("DD_"));
            if(validPatRow != null)
            {
        ...
        {
            Regex otpMapSizeReg = new Regex(@"otp_map_size[\s]*:[\s]*(?<value>[\d]+)", RegexOptions.IgnoreCase);
            Regex otpFullSizeReg = new Regex(@"otp_full_size[\s]*:[\s]*(?<value>[\d]+)", RegexOptions.IgnoreCase);
            string otpMapSize = otpMapSizeReg.Match(yamlVersionRow).Groups["value"].ToString();
            string otpFullSize = otpFullSizeReg.Match(yamlVersionRow).Groups["value"].ToString();
        ...
        private void readRegisterNames(out string otpEventFaultReg, out string tpVersionMsb, out string tpVersionLsb, out string svnVersionMsb, out string svnVersionLsb)
        {
            string canNotFindRegisterName = "Cannot find this register";
            ...
            //TPVERSION_MSB
            targetRegister = _otpReader.OtProws.Find(s => Regex.IsMatch(s.Name.Trim(), @"^MAJOR_[O]?TP_[RE]?VERSION$", RegexOptions.IgnoreCase));
            if (targetRegister != null)
                tpVersionMsb = targetRegister.OtpRegisterName;
            //TPVERSION_LSB
            targetRegister = _otpReader.OtProws.Find(s => Regex.IsMatch(s.Name.Trim(), @"^MINOR_[O]?TP_[RE]?VERSION$", RegexOptions.IgnoreCase));
            if (targetRegister != null)
                tpVersionLsb = targetRegister.OtpRegisterName;
            //SVN_VERSION_MSB
            targetRegister = _otpReader.OtProws.Find(s => s.Name.Trim().Equals("LCK1", StringComparison.OrdinalIgnoreCase));
            if(targetRegister == null)
                targetRegister = _otpReader.OtProws.Find(s => s.Name.Trim().Equals("ATE1", StringComparison.OrdinalIgnoreCase));
            if (targetRegister != null)
                svnVersionMsb = targetRegister.OtpRegisterName;
            //SVN_VERSION_LSB
            targetRegister = _otpReader.OtProws.Find(s => s.Name.Trim().Equals("LCK0", StringComparison.OrdinalIgnoreCase));
            if (targetRegister != null)
                svnVersionLsb = targetRegister.OtpRegisterName;
            //OTP_EVENT_FAULT_REG
            AhbRegRow targetAhbRegister = _ahbRegSheet.AhbRegRows.Find(s => s.FieldName.Trim().Equals("FLT_OTP_CRC", StringComparison.OrdinalIgnoreCase));
            if(targetAhbRegister == null)
                targetAhbRegister = _ahbRegSheet.AhbRegRows.Find(s => s.FieldName.Trim().Equals("EVT_CRC", StringComparison.OrdinalIgnoreCase));
            if (targetAhbRegister != null)
