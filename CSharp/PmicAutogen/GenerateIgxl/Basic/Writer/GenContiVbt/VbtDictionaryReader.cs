    ...
    public class VbtDictionaryReader
    {
        ...
        public List<Dictionary<string, string>> ReadSheet(ExcelWorksheet worksheet)
        {
            ...
            for (var i = 2; i <= _endRowNumber; i++)
            {
                ...
                {
                    var header = EpplusOperation.GetMergedCellValue(_excelWorksheet, 1, j).Replace(" ", "");
                    var content = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, j);
        ...
        private bool CheckHeader()
        {
            ...
            for (var j = 1; j <= _endColNumber; j++)
            {
                var content = EpplusOperation.GetCellValue(_excelWorksheet, i, j);
