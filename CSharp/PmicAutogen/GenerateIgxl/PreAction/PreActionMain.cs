    ...
    public class PreActionMain : MainBase
    {
        public void WorkFlow()
        {
            try
            {
                ...
                Response.Report("Generating PinMap ...", MessageLevel.General, 40);
                var pinMapMain = new PinMapMain(StaticTestPlan.IoPinMapSheet, StaticTestPlan.IoPinGroupSheet, StaticTestPlan.PortDefineSheet);
                ...
                Response.Report("Pre-Action Completed!", MessageLevel.General, 100);
            }
            catch (Exception e)
            {
                Response.Report("Meet an error in pre-action of autogen. " + e.Message, MessageLevel.Error, 0);
            }
        }
        ...
        public void ReadFiles()
        {
            ...
            Response.Report("Initializing VBT functions ...", MessageLevel.General, 40);
            var basMain = new BasMain(VersionControl.SrcInfoRows);
            basMain.WorkFlow(FolderStructure.DirLib);
        }
            ...
            if (Directory.Exists(extraFolder))
            {
                ...
                foreach (var extraTxt in mExtraList)
                    if (Regex.IsMatch(extraTxt.Name, "^Channel*", RegexOptions.IgnoreCase))
                    {
                        ...
                        channelMapMain.WorkFlow(extraTxt.FullName);
                    }
            }
        }
    }
