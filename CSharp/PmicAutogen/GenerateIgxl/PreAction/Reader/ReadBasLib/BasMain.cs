            ...
            while ((line = sr.ReadLine()) != null)
            {
                ...
                if (_functionNames != null)
                {
                    ...
                    else
                    {
                        ...
                        EpplusErrorManager.AddError(PreActionErrorType.DuplicateVbtModule.ToString(), ErrorLevel.Error,
                            "", 1, outString, fileName, _functionNames[functionName]);
                        Response.Report(outString, MessageLevel.Error, 0);
                    }
                }
        ...
        public void AddComment(string filePath)
        {
            if (File.Exists(filePath))
            {
                ...
                if (_srcInfoRows != null && index != -1)
                    newLines.InsertRange(index + 1, _srcInfoRows.Select(x => "'" + x.Comment + ":" + x.InputFile));
