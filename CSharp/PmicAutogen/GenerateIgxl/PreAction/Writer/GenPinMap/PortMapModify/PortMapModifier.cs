    ...
    public class PortMapModifier
    {
        public void WorkFlow(PortMapSheet portMapSheet, ref List<PortSet> validPortSets)
        {
            ...
            if (portDefineSheet != null)
            {
                ...
                {
                    if (pair.Value.Any(o => string.IsNullOrEmpty(o.Pin)))
                    {
                        return false;
                    }
                    return true;
                ...
                if (validPortDefineRows.Count() > 0)
                {
                    ...
                    {
                        var validPortDefineRow = validPortDefineRows.FindAll(o => o.Key.Equals(portSet.PortName, StringComparison.CurrentCultureIgnoreCase));
                        if (validPortDefineRow.Count() == 0)
                            continue;
                        foreach (var row in validPortDefineRow.First().Value)
                        {
                            if (portSet.PortRows.Exists(x =>
                           x.PortName.Equals(row.ProtocolPortName, StringComparison.CurrentCultureIgnoreCase) &&
                ...
                List<PortSet> newOrderPortSetList = new List<PortSet>();
                List<PortSet> invalidPortSetList = new List<PortSet>();
                ...
                foreach (var portSet in portMapSheet.PortSets)
                {
                    ...
                    if (validPortNameList.Any(o => o.Equals(portSet.PortName.Trim(), StringComparison.CurrentCultureIgnoreCase)))
                    {
                        newOrderPortSetList.Add(portSet);
                ...
                //add an empty row
                PortSet emptyPortSet = new PortSet("");
