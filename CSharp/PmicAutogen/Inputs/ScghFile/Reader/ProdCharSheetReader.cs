    ...
    public class ProdCharSheetReader
    {
        ...
        private List<string> ReadInitList(int rowNumber)
        {
            ...
            for (var i = 0; i < _initCnt; i++)
                initList.Add(EpplusOperation.GetMergedCellValue(_sheet, rowNumber, _initStartColNumber + i).Trim());
            return initList;
        }
        ...
        private List<string> ReadPayloadList(int rowNumber)
        {
            ...
            {
                var payload = EpplusOperation.GetMergedCellValue(_sheet, rowNumber, _payloadStartColNumber + i).Trim();
            ...
            for (var i = _startRowNumber + 1; i <= _endRowNumber; i++)
            {
                ...
                if (GetCellValue(i, _payloadStartColNumber) != "")
                {
                    ...
                    row.Block = _blockColNumber == 0
                        ? ""
                        : EpplusOperation.GetMergedCellValue(_sheet, i, _blockColNumber).Trim();
                    row.Mode = EpplusOperation.GetMergedCellValue(_sheet, i, _modeColNumber).Trim();
                    row.Item = EpplusOperation.GetMergedCellValue(_sheet, i, _itemColNumber).Trim();
                    ...
                    row.Application = _applicationColNumber != -1
                        ? EpplusOperation.GetMergedCellValue(_sheet, i, _applicationColNumber).Trim()
