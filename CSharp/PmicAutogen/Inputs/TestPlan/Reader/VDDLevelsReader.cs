...
namespace PmicAutogen.Inputs.TestPlan.Reader
{
    ...
    public class VddLevelsSheet
    {
        ...
        public bool ULvHasNA = false;
        public bool UHvHasNA = false;
        ...
        public List<GlobalSpec> GenGlbSymbol(IfoldPowerTableSheet ifoldPowerTable)
        {
            ...
            foreach (var row in Rows)
            {
                ...
                var plus_UHv = nv == 0 ? "0" : VariblePrefix + row.UHvRange + "/" + nvValue;
                var minus_ULv = nv == 0 ? "0" : VariblePrefix + row.ULvRange + "/" + nvValue;
                ...
                if (!UHvHasNA && row.UHv.Trim() != "")
                {
                    glbSymbol = SpecFormat.GenGlbPlusUHv(row.WsBumpName); // _GLB_Plus
                    globalPlusUHv.Add(new GlobalSpec(glbSymbol, SpecFormat.GenSpecValueSingleValue(plus_UHv)));
                }

                if (!ULvHasNA && row.ULv.Trim() != "")
                {
                    glbSymbol = SpecFormat.GenGlbMinusULv(row.WsBumpName); // _GLB_Minus
                    globalMinusULv.Add(new GlobalSpec(glbSymbol, SpecFormat.GenSpecValueSingleValue(minus_ULv)));
                }
        ...
        public List<DcSpecs> GenDcSymbol(List<DcCategory> categories)
        {
            var dcSpecList = new List<DcSpecs>();
            ...
            foreach (var row in Rows)
            {
                ...
                var dcSpec = new DcSpecs(dcSpecSymbol);
        ...
        private void SetDcSpecValue(VddLevelsRow row, ref DcSpecs dcSpec, List<DcCategory> categories,
            List<string> group)
        {
            ...
            try
            {
                foreach (var category in categories)
                {
                    ...
                    {
                        if (categoryName.EndsWith("UltraVoltage", StringComparison.InvariantCultureIgnoreCase))
                        {
                            categoryInSpec.Max = "0";
                            categoryInSpec.Min = "0";
                            categoryInSpec.Typ = "0";

                            if (!UHvHasNA && row.UHv != null && row.UHv != "")
                            {
                            ...
                            if (!ULvHasNA && row.ULv != null && row.ULv != "")
                            {
                                var glbSymbol = SpecFormat.GenGlbSpecSymbol(row.WsBumpName);
    ...
    public class VddLevelsReader
    {
        ...
        private VddLevelsSheet ReadSheetData()
        {
            ...
            for (var i = _startRowNumber + 1; i <= _endRowNumber; i++)
            {
                ...
                if (_wsBumpNameIndex != -1)
                    row.WsBumpName = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _wsBumpNameIndex).Trim();
                if (_lvIndex != -1)
                {
                    string lvRange = string.Empty;
                    row.Lv = EpplusOperation.GetMergedCellValueAndAddress(_excelWorksheet, i, _lvIndex, ref lvRange).Trim();
                    row.LvRange = lvRange;
                }
                if (_nvIndex != -1)
                {
                    string nvRange = string.Empty;
                    row.Nv = EpplusOperation.GetMergedCellValueAndAddress(_excelWorksheet, i, _nvIndex, ref nvRange).Trim();
                    row.NvRange = nvRange;
                }
                if (_hvIndex != -1)
                {
                    string hvRange = string.Empty;
                    row.Hv = EpplusOperation.GetMergedCellValueAndAddress(_excelWorksheet, i, _hvIndex, ref hvRange).Trim();
                    row.HvRange = hvRange;
                }
                if (_ulvIndex != -1)
                {
                    string ulvRange = string.Empty;
                    row.ULv = EpplusOperation.GetMergedCellValueAndAddress(_excelWorksheet, i, _ulvIndex, ref ulvRange).Trim();
                    row.ULvRange = ulvRange;
                }
                if (_uhvIndex != -1)
                {
                    string uhvRange = string.Empty;
                    row.UHv = EpplusOperation.GetMergedCellValueAndAddress(_excelWorksheet, i, _uhvIndex, ref uhvRange).Trim();
                    row.UHvRange = uhvRange;
                }
                if (_seqIndex != -1)
                    row.Seq = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _seqIndex).Trim();
                if (_commentIndex != -1)
                    row.Comment = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _commentIndex).Trim();
                if (_referenceLevelIndex != -1)
                    row.ReferenceLevel = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _referenceLevelIndex).Trim();
                if (_finalSeqIndex != -1)
                    row.FinalSeq = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _finalSeqIndex).Trim();
                ...
                if (string.IsNullOrEmpty(row.WsBumpName) && string.IsNullOrEmpty(row.Lv)
                   && string.IsNullOrEmpty(row.Hv) && string.IsNullOrEmpty(row.Seq) && string.IsNullOrEmpty(row.Comment))
                    break;

                foreach (var col in _vddLevelsSheet.HeaderList)
                {
                    var header = EpplusOperation.GetMergedCellValue(_excelWorksheet, _startRowNumber, col).Trim();
                    var context = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, col).Trim();
        ...
        private void PostAction()
        {
            ...
            var UHv_NA_Rows = _vddLevelsSheet.Rows.FindAll(row => row.UHv.Equals("NA", StringComparison.CurrentCultureIgnoreCase)).ToList();
            if (UHv_NA_Rows.Any())
                _vddLevelsSheet.UHvHasNA = true;

            var ULv_NA_Rows = _vddLevelsSheet.Rows.FindAll(row => row.ULv.Equals("NA", StringComparison.CurrentCultureIgnoreCase)).ToList();
            if (ULv_NA_Rows.Any())
                _vddLevelsSheet.ULvHasNA = true;
