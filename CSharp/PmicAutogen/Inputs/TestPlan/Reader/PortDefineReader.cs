...
namespace PmicAutogen.Inputs.TestPlan.Reader
{
    ...
    public class PortDefineSheet
    {
        ...
        {
            List<PortDefineRow> invalidRows = new List<PortDefineRow>();
            var portAndPinDic = GroupByPortName();
            foreach (var portAndPinItem in portAndPinDic)
            {
                List<PortDefineRow> emptyPinRows = portAndPinItem.Value.FindAll(o => string.IsNullOrEmpty(o.Pin.Trim()));
        ...
        {
            var fisrt = Rows.First(x => x.ProtocolPortName.Equals(name, StringComparison.CurrentCultureIgnoreCase));
            if (fisrt != null)
                return fisrt.Pin;
            return "";
        }
        ...
        private PortDefineSheet ReadSheetData()
        {
            ...
            for (var i = _startRowNumber + 1; i <= _endRowNumber; i++)
            {
                ...
                if (_protocolPortNameIndex != -1)
                    row.ProtocolPortName = EpplusOperation
                        .GetMergedCellValue(_excelWorksheet, i, _protocolPortNameIndex).Trim();
                if (_typeIndex != -1)
