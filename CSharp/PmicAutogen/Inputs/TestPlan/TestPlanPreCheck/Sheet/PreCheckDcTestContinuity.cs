...
namespace PmicAutogen.Inputs.TestPlan.TestPlanPreCheck.Sheet
{
    public class PreCheckDcTestContinuity : PreCheckBase
    {
        ...
        protected override bool CheckBusiness()
        {
            ...
            {
                var header = EpplusOperation.GetMergedCellValue(Worksheet, StartRow, j);
        ...
        private bool CheckConditionFormat(int column)
        {
            ...
            for (var i = StartRow + 2; i <= Worksheet.Dimension.End.Row; i++)
            {
                var value = EpplusOperation.GetCellValue(Worksheet, i, column);
                if (value == "")
                    return true;
                ...
                if (Regex.IsMatch(value, pattern) == false)
                {
                    ...
                    EpplusErrorManager.AddError(BasicErrorType.FormatError, ErrorLevel.Error, SheetName, i, column,
                        errorMessage);
                }
        ...
        private bool CheckLimitFormat(int column)
        {
            ...
            var mergeCellCnt = 2;
            var range = Worksheet.MergedCells[StartRow, 1];
            if (range != null)
            {
            ...
            for (var i = StartRow + mergeCellCnt + 1; i < Worksheet.Dimension.End.Row; i++)
            {
                var value = EpplusOperation.GetCellValue(Worksheet, i, column);
                if (value == "")
                    return true;
