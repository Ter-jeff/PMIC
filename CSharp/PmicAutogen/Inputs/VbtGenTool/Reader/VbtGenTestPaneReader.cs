    ...
    public class VbtGenTestPlanSheetReader
    {
        ...
        private VbtGenTestPlanSheet ReadSheetData()
        {
            for (var i = _startRowNumber + 1; i <= _endRowNumber; i++)
            {
                ...
                row.RowNum = i;
                if (_topListIndex != -1)
                    row.TopList = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _topListIndex).Trim();
                if (_commandIndex != -1)
                    row.Command = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _commandIndex).Trim();
                ...
                if (_registerMacroNameIndex != -1)
                    row.RegisterMacroName = EpplusOperation
                        .GetMergedCellValue(_excelWorksheet, i, _registerMacroNameIndex).Trim();
                if (_bitfieldNameIndex != -1)
                    row.BitfieldName = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _bitfieldNameIndex)
                        .Trim();
                if (_valuesIndex != -1)
                    row.Values = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _valuesIndex).Trim();
                if (_pinIndex != -1)
                    row.Pin = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _pinIndex).Trim();
                if (_datalogVariableIndex != -1)
                    row.DataLogVariable = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _datalogVariableIndex)
                        .Trim();
                if (_unitIndex != -1)
                    row.Unit = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _unitIndex).Trim();
                if (_lowLimitIndex != -1)
                ...
                if (_callBackFunctionIndex != -1)
                    row.CallbackFunction = EpplusOperation
                        .GetMergedCellValue(_excelWorksheet, i, _callBackFunctionIndex).Trim();
                if (_vRangeLIndex != -1)
                    row.VrangeL = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _vRangeLIndex).Trim();
                if (_iRangeLIndex != -1)
                    row.IrangeL = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _iRangeLIndex).Trim();
                if (_voltageLIndex != -1)
                    row.VoltageL = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _voltageLIndex).Trim();
                if (_currentLIndex != -1)
                    row.CurrentL = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _currentLIndex).Trim();
                if (_vRangeMIndex != -1)
                    row.VrangeM = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _vRangeMIndex).Trim();
                if (_iRangeMIndex != -1)
                    row.IrangeM = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _iRangeMIndex).Trim();
                if (_voltageMIndex != -1)
                    row.VoltageM = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _voltageMIndex).Trim();
                if (_currentMIndex != -1)
                    row.CurrentM = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _currentMIndex).Trim();
                if (_frequencyIndex != -1)
                    row.Frequency = EpplusOperation.GetMergedCellValue(_excelWorksheet, i, _frequencyIndex).Trim();
        ...
        private bool GetFirstHeaderPosition()
        {
            ...
            for (var j = 1; j < colNum; j++)
                if (EpplusOperation.GetCellValue(_excelWorksheet, i, j).Trim()
                    .Equals(HeaderTopList, StringComparison.OrdinalIgnoreCase) && EpplusOperation
                    .GetCellValue(_excelWorksheet, i, j + 1).Trim()
        ...
        private bool GetFlowPosition()
        {
            ...
            for (var j = 1; j < colNum; j++)
                ...
                {
                    var flow = Regex
                        .Match(EpplusOperation.GetCellValue(_excelWorksheet, i, j).Trim(), reg1,
